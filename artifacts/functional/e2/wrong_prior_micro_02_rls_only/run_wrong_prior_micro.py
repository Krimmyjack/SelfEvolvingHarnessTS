"""Wrong-prior micro on e1v2_task_02 only.

Pure open-loop RLS/level Source prior. MUST NOT name structural, outlier,
or impute in the Source skill/prior text or family list. One arm only.
A5-style frozen Source Skill inject, then live Fast inspect+propose
(run_agentic_fast_path generate stages). No 01. No matching-prior arm.
No Support/select. No M0b. No Observation/Skill stack. No electricity.
No SelfEvolvingHarnessTS filesystem junction. Gate untouched.
"""
...
TASK_IDS = ("e1v2_task_02",)
COHORT = "e31"
RUN_REL = Path("artifacts/functional/e2/wrong_prior_micro_02_rls_only")
STATE_REL = str(RUN_REL / "state")
LLM_CAP = 12
TOOL_CAP = 6
FORBIDDEN_PRIOR_TOKENS = ("structural", "outlier", "impute")

# Pure open-loop RLS / level-only. Do not name structural, outlier, or impute.
WRONG_PRIORS = {
    "e1v2_task_02": {
        "try": "Lead with repair_level_shift as the sole open-loop level prior.",
        "family": "level",
        "operator": "repair_level_shift",
        "label": "open-loop repair_level_shift / level-only",
    },
}
...
def _sections(try_text: str) -> dict[str, str]:
    return {
        ...
        "FALLBACK": "Keep an effect-distinct candidate supported by the public inspect.",
    }


def _assert_rls_level_only(prior: dict[str, Any]) -> None:
    blob = " ".join(
        [
            str(prior.get("family") or ""),
            str(prior.get("try") or ""),
            str(prior.get("label") or ""),
            *(_sections(prior["try"]).values()),
        ]
    ).lower()
    bad = [tok for tok in FORBIDDEN_PRIOR_TOKENS if tok in blob]
    if bad:
        raise SystemExit("prior is not RLS/level-only: %s" % (bad,))
    if "repair_level_shift" not in str(prior.get("try") or "").lower():
        raise SystemExit("prior TRY must name repair_level_shift")
    if str(prior.get("family") or "") not in {"level", "repair_level_shift"}:
        raise SystemExit("prior family must be level or repair_level_shift")
...
def _run_one(
    *,
    ...
    rls_ops = [op for op in first_ops if op]
    monopolized = bool(rls_ops) and set(rls_ops) == {"repair_level_shift"}
...
def _score(rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    monopolized = any(
        bool((rows.get(tid) or {}).get("rls_monopolized_k"))
        or bool((rows.get(tid) or {}).get("wrong_family_monopolized_k"))
        for tid in TASK_IDS
    )
    as_confirm = any(
    ...
    if monopolized:
        reasons.append("RLS monopolized K")
    if as_confirm:
...
def main() -> int:
    ...
    scoring = _score(rows)
    report = {
        "kind": "wrong_prior_micro_02_rls_only",
        "arm": "A5",
        "cohort": COHORT,
    ...
    monopoly_word = (
        "有"
        if scoring["rls_monopolized_k"]
        or scoring["wrong_prior_treated_as_confirmation"]
        else "无"
    )
    attached = WRONG_PRIORS["e1v2_task_02"]
    lines = [
        "e1v2_task_02 门: %s" % _gate_word("e1v2_task_02"),
        "K: %s" % (rows["e1v2_task_02"].get("families") or []),
        "RLS独占K/当确认: %s" % monopoly_word,
        scoring["verdict"],
        "prior family/text: %s / %s" % (attached["family"], attached["try"]),
    ]
