"""A3-only Fast inspect+propose micro on e1v2_task_01/02 (no 08).

Uses the live Fast generate path (family menu + _validate_propose).
Empty candidates raise retryable FAMILY_EMPTY_PROPOSE (not abstain-success).
No A5, no Slow, no Support/select, no electricity AB. No SelfEvolvingHarnessTS junction.
"""
...
_argv2 = list(sys.argv[1:])
while _argv2:
    if _argv2[0] == "--root":
        _argv2 = _argv2[2:]
    elif _argv2[0] == "--tasks":
        TASK_IDS = tuple(x for x in _argv2[1].split(",") if x)
        _argv2 = _argv2[2:]
    else:
        break
COHORT = "e31"
RUN_REL = Path("artifacts/functional/e2/proposer_micro_0102_noempty")
STATE_REL = str(RUN_REL / "state")
# inspect + propose + schema retry each, plus a few tool rounds
LLM_CAP = 12
TOOL_CAP = 6
...
def _codes_of(row: dict[str, Any]) -> list[str]:
    codes = list(row.get("validation_error_codes") or [])
    proto = str(row.get("protocol_error") or "")
    if "FAMILY_EMPTY_PROPOSE" in proto and "FAMILY_EMPTY_PROPOSE" not in codes:
        codes.append("FAMILY_EMPTY_PROPOSE")
    return codes
...
def _score(per_task: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    t02_has_non_rls = False
    all_families: set[str] = set()
    empty: list[str] = []
    empty_exhausted: list[str] = []
    for tid, row in per_task.items():
        ...
        if not families:
            empty.append(tid)
            if "FAMILY_EMPTY_PROPOSE" in codes or not row.get("propose_entered"):
                empty_exhausted.append(tid)
    ...
    if not t02_has_non_rls:
        reasons.append("02 缺非RLS")
    if len(all_families) < 2:
        reasons.append("合计不足 2 家族")
    if empty:
        reasons.append("empty K / FAMILY_EMPTY_PROPOSE:" + ",".join(empty))
    passed = not reasons
    return {
        "pass": passed,
        ...
        "task_02_non_rls": t02_has_non_rls,
        "all_same_family": bool(monopoly_tasks),
        "all_families": sorted(all_families),
        "empty_tasks": empty,
        "empty_exhausted": empty_exhausted,
        "fail_rules": reasons,
...
def _run_one(
    *,
        ...
        propose_input = {
            **inspect_input,
            ...
            "stage_note": (
                ...
                "legal propose completion (FAMILY_EMPTY_PROPOSE). You choose "
                "the Program family; the Runtime owns where inside each action "
                "unit the edit lands. Each candidate costs one Target Support "
                "probe from a non-renewable budget." % int(B)
            ),
        }
    ...
    return {
        ...
        "retries_happened": retries_happened,
        ...
        "raw_candidate_types": [type(row).__name__ for row in raw_candidates],
...
def main() -> int:
    ...
    all_score_ids = ("e1v2_task_01", "e1v2_task_02")
    for tid in TASK_IDS:
        ...
        rows[tid] = row
        print(
            "MICRO_TASK_DONE %s families=%s ops=%s raw_n=%s keys=%s propose_entered=%s retries=%s proto=%s infra=%s llm=%s"
            % (
                tid, row.get("families"), row.get("first_ops"),
                row.get("raw_candidate_count"), row.get("propose_payload_keys"),
                row.get("propose_entered"), row.get("retries_happened"),
    ...
    for tid in all_score_ids:
        if tid in rows:
            continue
        prior = run_dir / ("%s.json" % tid)
        if prior.is_file():
            rows[tid] = json.loads(prior.read_text(encoding="utf-8"))
    scoring = _score(rows)
    report = {
        "kind": "proposer_micro_0102_noempty",
        "arm": "A3",
        "cohort": COHORT,
        ...
        "no_slow": True,
        "no_a5": True,
        "runner": "run_proposer_micro.py (A3 Fast inspect+_run_stage propose / _validate_propose)",
        "rows": rows,
        "scoring": scoring,
    ...
    lines = [
        "e1v2_task_01: %s" % (rows["e1v2_task_01"].get("families") or []),
        "e1v2_task_02: %s" % (rows["e1v2_task_02"].get("families") or []),
        "e1v2_task_08: 未测",
        "全员同一家族: %s" % ("有" if scoring["all_same_family"] or scoring["empty_tasks"] else "无"),
        "02 非RLS: %s" % ("有" if scoring["task_02_non_rls"] else "无"),
        scoring["verdict"],
        "runner: run_proposer_micro.py A3 Fast inspect+propose/_validate_propose; propose_entered=%s/%s retries=%s/%s"
        % (
            rows["e1v2_task_01"].get("propose_entered"),
            rows["e1v2_task_02"].get("propose_entered"),
            rows["e1v2_task_01"].get("retries_happened"),
            rows["e1v2_task_02"].get("retries_happened"),
        ),
    ]
