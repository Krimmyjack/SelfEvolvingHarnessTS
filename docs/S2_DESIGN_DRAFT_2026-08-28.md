# S2a 设计稿:机制跨任务可移植性 + 跨任务隔离(2026-08-28,主线定稿;sol 已核)

> **sol 核准记录(2026-08-28 16:4x)**:方向批,**定名 S2a**——本设计证"同一机制服务不同任务"(预测 Skill 系预测任务内重学)+"任务边界有效"(分类卡沉默),**不得写成完整"跨任务知识迁移"**;任务中立程序知识的正向迁移(例:"先查反馈可读性、证据不足弃权"→减少无效 probe/降适应成本)系缓议 G3,预测闭环跑通后另做小型 **S2b**;S2a+S2b 合起来才闭合 Stage 2 多任务承诺。v1.1 受限修订案批;两掷帽合理,仍空则候选发现正式移交 Stage 3,不再改课。
