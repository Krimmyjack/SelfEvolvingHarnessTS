...
<!-- ANCHOR:REVIEW  第二审查线只在此行之下、下一锚点之上插入 -->

<!-- ANCHOR:OPUS  Opus/执行线只在此行之下、下一锚点之上插入(仪器条目) -->

### sol 确认 P0 修法并关闭 gate 裁定;h0 lock 仅轮转一次;`harness_content_sha` 不变;非作者增量复核 PASS(2026-09-03 21:xx,执行方/Opus)

**裁定闭合**:P0 修法维持加法式 `authorize`——在 `handle_feedback_delayed` **写 snapshot 之前**询问,False 则丢 pending、不写、记 `authority_refused`;不用事后恢复。gate 四规则维持原码:`AUTHORITY_BYPASSED`(任一状态泄漏或风险线分歧)降级;`AUTHORITY_UPHELD`(仅 `coverage_floor` 且状态完整不变)只披露;`LOST_ACTIVATION` 只计数。不再等新方法裁定。

**lock 一次**(授权后、代码固定后、仅此一次):`methods/ttha/harness/h0/snapshot.lock.json`。`harness_content_sha` **逐字节不变** `53b1c803f4ba38a27e2d1d7621f983997044019d7e073caef2a4436ee900654f`;`runtime_bundle_sha` `c3427b4e…` → `29298dc9…`。移动的 dependency 键 6 个:`ttha:method`(本 P0)、`compiler_source` / `schema:skill_entry_v1` / `ttha:fast_agent` / `ttha:schema_contracts`(HEAD `7b3cba8` 既有漂移)、`agent_schema:slow_scope_clause_v1`(新键)。`compile_snapshot(verify_lock=True)` 现通过。

**测试**:HEC-1 聚焦 `123 passed / 1 skipped`;smoke **7/7**;指定 integration 修 lock 前 `FAILED` 于 `snapshot lock mismatch`,修后 `compile_snapshot(verify_lock=True)` 通过——原失败点已消。无线程帽的隔离跑在 scipy `lstsq`/`savgol` 撞已知 OMP abort(仪器债,非 lock,栈从未回到 `method.py`)。全量差集与 KMP 隔离重跑在 commit 时仍在跑(stdout 块缓冲,进程 CPU 持续上升)。

**非作者增量复核**:[P0 increment review](79c00714-feca-4fc1-a1e3-cee266573f03) **PASS**(A–H 全过,无阻塞项)。披露(不阻塞):`authority_state` 包住整个 `open_delayed`;下一单元锁看 `deployed_via` 不看空检索集;authorizer 用 runner 预计算的 P4 `_gate`。

**0 LLM**。下一步:allowlist commit(含 `method.py` / `online_loop.py` / lock)→ 收据记 commit(不自指)→ Phase S-v1.1。

<!-- ANCHOR:MAINLINE  主线只在此行之下插入;各线不得动他线条目(所有权表见 docs/STATE_ONE_PAGE_2026-09-03.md) -->

### DEV-AUTO-1 主线读数:提议端首次在自然环路活了(25 个合法提议、失败改变下一步提议 12/13),端到端 0 接受 0 更新;"5 个更好被错杀"经共同人群复算不成立(第三次分母错位);接受框架不对称为真;一条 LLM 自提的 Workflow 线索待前向验证(2026-09-06 夜,主线入典;0 LLM / 0 fit / 0 代码;未 commit)

**事实(Opus 报告 + astra 复算 + 主线核代码)**:两条两臂开发课程(run1 中止:schema 允许无载荷提议;run2 为读数),828/1000 fits,DeepSeek 端点(agicto 中继对 >4.4k token 返回 403 → 完整行提示需长上下文),绝对数不与 HEC-1 并表。真实 Slow 在放开的外环角色 + 两出口下 16 步提 25 个合法修改(10 个不同 Workflow、3 个 Scope 方向),弃权 2 次不终止,阈值全由 `calibrate` 定;**Q2 序列**:u6 `hampel_filter`(最坏 2.16、受损比 0.7)→ u8 换 `winsorize`(0.3);u12 `outlier_iqr` 超父但撞风险线 → u16 `outlier_iqr>winsorize`;u9 `denoise_median` 窗口验证器全拒 → 后续不再出现;相邻步提议集合 12 改 1 同——首次观察到"失败反馈以合理方向改变下一提议"(响应性,非学习)。**0 接受**:`outer_loop.screen` 绝对门(任一适用已处理单元越线即淘汰),持权卡免检,父版本 16/16 步过不了该屏 ⇒ 父版本失败的谱系上任何修订结构上不可接受。**分母错位(第三次)**:`dev_auto1_revision.py:431` 效用均值只在可读格上算,候选不可执行格被丢弃而非按 raw 回退记 0(候选 7 格 vs 父 15 格);统一到 15 格后 u16 候选 +0.125 vs 父 +0.166;"超父 ≥ material" 9 → 3,"两风险汇总不差" 5 → 0 ⇒ **"放宽屏就能接受"无证据**,正确相对屏亦不接受那五个。三次同类错误(工具/门分母;Support/delayed 面;候选/父格集)⇒ **全项目统一一个比较器**(同单元集、同服务人群、既定回退、缺测单列不记零)。**线索**:`outlier_iqr>winsorize`(u16/u24 两次自提、两次被拒)在首次提出后 4 个真正后续单元:+0.367 vs 祖先 +0.286、权威门 3/4 vs 0/4、受损 13 vs 12、最坏 1.14 vs 1.12——首条系统自生、事后优于父的 Workflow 差分,**预注册为下次 live 候选,不作证据**。**其余更正(astra)**:两臂不只差 Skill(冻结臂每格重建 Draft 状态,修订臂携带并于 u12 FLAGGED,供给不同;run1/run2 数字对调 = 依赖 16 格中仅 2 格部署 `outlier_mad`,非纯噪声)→ 下一版只冻 Skill 更新;Scope 8 次失败 = 4 `level_excursion_score` / 3 `local_robust_z_peak` / 1 `period_reliability`(分箱正常仍撞伤害线),不全归栅格;+0.407 探针受损比 0.25 > 0.20 拒绝合理,真正障碍是 9/32 格被每格 5 次 Fast 帽截断(包级预算未落到 Fast);Support 表 `NO_STORED_READING` 被记零须改缺测。**主线补充判断**:绝对屏方向错——修订须过全部历史、新候选只需前向 Support→delayed;父免检太松、修订回看全史太严;更一致的框架 = 修订与新候选走同一前向验证路径,历史屏只排序不淘汰(接受严在前向独立验证)。**下一步(采 astra 短补修包 + 两条)**:统一比较器 / 冻结臂只冻 Skill / Fast 预算两臂包级 / 修后在 run2 的 25 个候选记录上离线比较三种接受框架(绝对屏 / 相对当时可执行策略 / 前向验证)是否产生真实分流,有分流才重跑;加读数"上一步失败类型 → 下一步提议如何变";`outlier_iqr>winsorize` 预注册。判词改写:`PROPOSAL_REACHED; ACCEPTANCE_FRAME_ASYMMETRIC; NO_CANDIDATE_MEETS_RELATIVE_FRAME_ON_COMMON_POPULATION; ONE_LEAD_PENDING_FORWARD_VERIFICATION`。

### 用户设计取向"简单而有效、提议宽、少门"采认;主线回答四问(逐序列反馈 / 弃权 / 归因 / Scope)并出外环 Slow 五处改动提案(计划 §4.6);核实 m_r0f:弃权很大程度是角色说明教出来的,提议失败源于 6 个分数型特征栅格错配(2026-09-06 夜,主线入典;0 LLM / 0 fit / 0 代码;未 commit)

**四问答**:① 反馈是逐序列的(同窗对照管线 vs 候选管线,同一真实未来打分,\(g_i\) < −0.005 记受损,`run_hec1.py:521-531`),结果信息细、时点有限、原因信息少、序列共享模型非独立;**奇怪之处不在反馈密,而在提议者几乎看不到它**(提示 `rows[:60]`、无身份、仅 Support、失败只回一行);应用前提:下游只返总分时逐序列风险线不能原样用。② "没改成"分四类,历史 0 修订几乎全属运行时提前挡住;真正的模型弃权(HEC-1 一次、ACC-1 一次)有具体线索——复用内环 preparation 角色说明("never infer candidate utility / inspect first / abstain"),m_r0f 只换外环角色说明、同证据 → 两次提议(n=1 对 n=1);两次提议败于**栅格错配**:12 词表中 6 个 [0,1] 分数型特征走默认分箱 (0,1,3,6),有效切点仅 0.0,96 三元组仅 8 可行,模型两次机会、反馈一行。Self-Harness 亦允许 decline,但默认 4 提议位、占位无效、弃权须证据理由(`multi_proposer.py:56-125`)——我们把默认设反了。③ 归因两套:五类选择题模块(`fault_cases.py`,防强行归因)HEC-1 未运行;实际决定 Draft 的是三态启发式,FLAGGED 持否决且无出口;`attribute()` 不拿上次增益,`sign_flipped_continuing` 仅表"以前处理过的成员本次为负"。Self-Harness / GEPA / AFlow 的归因 = 模型诊断假说 + 测试裁决,不先证因果。**问题不是归因粗,是粗归因拿了否决权。** ④ Scope = 12 特征 ≤2 AND 子句冻结栅格谓词 + 人工冻结初始化表(非 Agent 学得)+ 独立 raw 对照管线;理论基础只是"条件化处理"的方法假设 + 固定程序下的精确代数(收窄效用变化 = −被排除增益和/N;嵌套下受损不增),提供作用边界与可比动作空间,不保证可学与安全;其表示(词表)与执行基础(栅格)都有具体缺陷。

**主线取向 = 提议宽、接受严、停止简单**:提议侧只留"合法、非重复、预算未尽";接受侧留四条(不偷看未开放答案 / 模型不改裁判 / 未验证不部署 / 发车前冻结);预算退到包级总上限 + 可中断,臂间比较时包级对齐;不设门但**全记**;放宽后的过拟合风险由接受侧与冻结防,不由提议侧加门。**计划 §4.6 五处改动(呈 sol)**:让它看见(完整行、身份、Support+delayed 作历史、失败后果)/ 换问法与外环角色 / 修 6 个分数型特征栅格(版本化)/ FLAGGED 去否决改顺序提示 / 预算退包级。第一包新增"提议者试跑"读数(提议 / 可行 / 过接受 / 后续有价值,与确定性并排)。

### 6 pro 三段分析采认:计划 §2"减法修订不可能创造新收益"数学不成立、撤回;新增可学习对象"修订效应 \(d_e(P\to P')\)"、面向动作观测 \(\psi(x,P,\text{Consumer})\)、新会话再生成检验、普通反思对照臂、A5 结构冻结变体;ACC-1 事实归位(真实 Slow 弃权;Slow 输入截断 60/100 行进接线修复;纵向评的是确定性修订)(2026-09-06 晚,主线入典;0 LLM / 0 fit / 0 代码;未 commit)

**事实归位(m_r0d attempt3 / m_r0e)**:修好接线后候选送达真实 Slow(gpt-5.6-sol),Slow 弃权 `insufficient_public_evidence`,无子句、无修订、无状态写入,1 次物理调用、0 fits;止步点前移到"Slow 愿不愿提"。非作者复核发现 `run_hec1.py:1665` 只把前 60 行 bank 入提示、`outer_loop.py:929` 把全量 100 行给影子——两侧输入不同,"同样证据下不愿提"不成立;m_r0e:60 行仍有 9 条可行三元组、最优 stump 同 `z_peak>=6.0`,截断削弱伤害证据量但未切掉可行信号,模型未在完整 100 行上再问。**Slow 输入截断进最小接线修复**;修好前不作"LLM vs 确定性"判词。m_r0j 纵向评价的 `missing_fraction>=0.05` 为 m_r0i 确定性对照所选(0 LLM),非 LLM 修订。

**6 pro 采认(计划 §2 更正、§7.0 三包增补)**:① §2 原句"减法修订不可能创造新收益"撤回——固定 Program、嵌套 Scope、退出回 raw 下 \(U(P,S')-U(P,S)=-\frac1N\sum_{i\in S\setminus S'}g_i\),被排除集合净负即有益;受损数下降是构造性的;仍成立的是迄今修订从未按全人群目标选择过;② **新可学习对象**:修订效应 \(d_e(P\to P')=L_e(P)-L_e(P')\) 是否比程序效应 \(g_e(P)\) 更稳定——Skill 可积累"P 出现何种可观察失败时先查哪环、比什么对照、何种修改曾有效、哪些部分不该丢",第一包对少量固定差分测跨单元符号一致率(Scope 差分 0 fit);③ **面向动作的观测** \(\psi(x,P,\text{Consumer})\)(修改连续成段 / 集中近期 context / 与缺失重叠),只由 context 与合法试执行算,版本化词表扩展并冻结分箱,测对 \(d_e\) 增量,无增量即停;同谱系 (P,S)×(P′,S′) 2×2 含交互项只作解释;④ 第二包加**普通 LLM 反思臂**(同信息量,先比整套机制再消融信息与使用)与**新会话再生成检验**(旧 Skill vs 新 Skill 各让 Fast 重生成程序,区分未用 / 用了行为未变 / 变了无助益;Runner 不直供赢家;引 *Harness Updating Is Not Harness Benefit*);解释类型 ≤5、每失败 ≤2 差分(一 Scope 一 Workflow),发车前冻结;⑤ 第三包加 A5 结构冻结只本地校准变体;⑥ 纪律:delayed 反馈时间角色(用于构造新修改后不得再作其独立验证,§4.3 采此措辞);研究者修评价分母 / 泄漏边界 / 成本计量非作弊,替系统逐次选答案才是;序列次数非独立样本,不以后来过门倒推当时可部署。**主线保留**:解释—对照—修订循环取最小版,不长成归因系统;\(d_e\) 的稳定性对 Scope 差分即被排除集合构成的稳定性,绕不开 D6 持续性,要测不预设;ψ 是能力扩展须版本化。研究中心(astra / 6 pro 收敛):反例到来后系统能否决定保留什么、修改什么并由后续证据检验;不赌 Scope 或 Workflow 谁强。

### astra 三审采认:证据成熟度措辞收回("只差一个正结果"撤);准入规则改决策论形式(比较对象 = 当时可执行策略,两面硬门撤);三分母拆开;后续固定为三个完整工作包;研究目标上抬为"下游反馈驱动、可修订、可迁移的数据准备能力",证据形态 = 四个行为差异(2026-09-06,主线入典;0 LLM / 0 fit / 0 代码;未 commit)

**采认与更正**:① "论文骨架已在、只差一个正结果"收回——基础环节已证(确定性对照下候选可校准/合法性/replay/写入 Draft;父子可在后续单元取真实配对读数),未证的是 LLM 自主完成有效修订链、修订经真实生命周期验证晋升并在后续部署获益、系统自动识别不值得的修订(现为研究者分析结论,非系统能力);② 准入规则:效用排序分母 = 全服务人群(对齐);风险约束人群是否统一为独立方法变更,单列分析;比较对象 = 当时真正可执行的策略(父版本按当时合法反馈可部署 → 比父;不可部署 → 比 raw 并报恢复可部署性),"Support 与 delayed 均不退步"硬门撤回(数据不支持且误伤风险修复);③ "别人都靠便宜即时评价"收窄(DGM ≈ $22k/次;AegisTS 代理+终局;Evo-Harness/TimeClaw 已做噪声经验→Skill),更有辨识度的问题 = 反馈获得时点 / 处理作用范围 / 效用统计范围三者不一致下如何积累程序性知识并让修改在后续场景改善可部署策略;Scope 搜索经 `ReplayPredictionCache` 精确复用、成本可摊薄,本轮评价器已把父子差异测清,失败在选择器未按最终差异选;④ 三层 oracle 须定义为可比量(同约束、固定/逐单元、含不修订与回退、同人群计分),L2−L3 为定位工具非定罪工具;TTHE 50%→64% 系题目准确率差,非"有好候选只选中一半";⑤ 失败档案的价值在下一轮行为是否改变;**时间纪律**:看过 pos 6/9/12/14 后设计的候选只测 L2,不得塞回 k1;⑥ "Scope 弱 / Workflow 强"维持为假设——Workflow 自由度大既可能修好也可能带来更大伤害、更高评价成本、更难泛化,并行的理由是干预能力不同。

**研究目标上抬(astra;主线采)**:不是"造了一个严谨、偶尔能修订的系统",而是"数据准备可以成为由下游反馈驱动、能够修订并迁移的能力";目标形状 = 三情境(形成工作流 → 新情境发现伤害并修订 → 第三未参与开发情境:冻结知识重犯旧错 / 无积累重新探索 / 修订知识少试错找到安全有效处理);**核心机制须解释增量从何而来**,证据 = 四个行为差异(有失败经验 → 更有针对性修订;去掉/打乱 → 优势消失;修订知识到后续场景仍有用;同预算重搜追不上)。若实现仍是小菜单 + 按历史分选 + 加门 + 存 Skill,即便正结果方法贡献亦有限;若第二包中去掉经验优势不消失或同预算搜索追平 → 方法贡献不足,调机制或定位,不以审核与日志包装。

**三个工作包(执行计划 §4.5 已改写)**:第一包 Scope 线 / Workflow 线可比开发比较,六列(含对齐仅效用分母、对齐+风险分母统一独立列、三层 oracle)+ 逐单元约束账 + 排除精度/代价,只答三问(合法空间有无更好修改 / 过去证据能否选出 / 价值类型),包末路线判断;第二包 A/B/C + 消融(B 去掉/打乱失败档案;同预算 validation-search),LLM 职责 = 从失败证据提有针对性程序性修改不猜阈值,修订须经真实验证晋升后续使用;第三包 A5/A3/Static 三情境(KDD → 第二情境 → Wind Farms),Solar 不预写弃疗集,NOAA 效率不作唯一主终点。**现在申请**:Wind Farms 下载、NOAA→HEC-1 适配器(第三包关键路径,不阻塞第一包)。节奏:每包主线一次定目标与方法,Opus/Kimi 完整执行,astra 只复核改变结论的问题与整包结果。

### astra 三处修正采认 + 两份机制文献报告读毕;"Scope 弱维度 / Workflow 强维度"降为**假设**;阻断原因转移已由主线从 m_r0j 收据复算证实;下一包加三层 oracle 与逐单元约束账,Workflow 候选并行准备但不同时改变机制(2026-09-06,主线入典;0 LLM / 0 fit / 0 代码;未 commit)

**复算(后 16 单元,delayed 面)**:祖先 伤害线(hf/msh)10/16、覆盖 3/16、全过 4/16;修订 伤害线 7/16、覆盖 6/16、全过 4/16——与 astra 一致。逐单元:子句在 pos 6/9/12/14(祖先撞 hf/msh,treated 20/17/12/20)**未改变任何选择**;pos 13/17 动了但仍撞 msh;pos 23/24/25 风险由不合格变合格,代价是 treated 4/0/3 → 撞覆盖线。**结论**:"过门数没增加"不能单归"特征认不出受害者";本轮既损收益,也把部分风险阻断变成覆盖阻断;"祖先其余 12 次都不安全"不准确(10 次风险、2 次仅覆盖)。

**三处修正采认**:① 阻断原因转移(上);② oracle 分三层——L1 逐序列事后 oracle(任意排除)/ L2 现有合法候选空间内事后最好(受词表、谓词、覆盖、风险约束)/ L3 只看边界前证据的选择器;L1 大 L2 小 → 表示/编辑空间受限;L2 大 L3 小 → 选择/估计/跨窗泛化;两者皆小 → 该谱系收窄不值得投入;"oracle 大但选不出"不证明异质性不可观测;上界不只看平均收益,让危险部署变合格也算价值;③ Workflow 证据是**局部、条件性**的(正典 §5.1:仅 2/6 origin 稳定、无程序跨 origin 稳定、且来自训练语料策展设置),主线此前"Workflow 是强维度"**降为假设**,Scope 上界不作 Workflow 设计的硬前置,两面并行准备、不同时改变实验中的多个机制。宏观:安全治理不天然只是 L1,换程序不自动更高级;可迁移的 Scope / 禁忌 / Workflow 发生结构修订并在后续独立场景产生价值,都服务项目目标。**当前困难的准确概括(astra)**:存在自然数据收益,但尚未找到一种修改方式能可靠地保留收益、控制风险并获得足够有效覆盖。

**文献(`OVERALL_MECHANISM_LITERATURE_BREADTH_2026-09-05.md`、`SelfEvolvingHarnessTS:整体机制与研究定位文献调研`)要点**:进化系统普遍遇到"修订 ≠ 改善"(DGM 谱系两次掉分;AFlow 列退步案例;TTHE 选中 50% vs 事后可选 64% → 好候选已生成却未被选中;Rethinking… 匹配预算后进化未稳定胜过多次尝试);它们的解法是**与父版本比较且两子集不退步**(Self-Harness)、**开放档案容许垫脚石**(DGM)、**把失败的具体后果喂给下一轮**(AFlow)、**分开生成与选择并量其差**(TTHE);几乎全部依赖即时可核验分数,本项目的延迟、噪声、干扰型 Consumer 评价器是最大错位——瓶颈很可能在评价器/环境而非提议器。报告提出三条互斥路线(A 开放式结构进化 / B 证据→可证伪 procedural Skill 编译与修订 / C Consumer 几何优先的集合级 Workflow 进化)与"三个单位对齐"(修订单位 / 干预单位 / 评价单位)。主线倾向(非裁定):Skill 对象取 B、pooled Consumer 下的干预几何取 C(集合级 Workflow 作生成型编辑,Scope 收窄作减法型编辑)、A 只取"失败 Draft 是探索材料"(生命周期已不删 Draft);不做整 Harness 自改。

**下一包增补(不改五列比较本体)**:逐单元约束账(收益 / 伤害线 / 覆盖分列)替代单一过门计数;三层 oracle;排除精度与代价;**选择器缺口 = L2 − L3**(TTHE 口径);修订准入取 Self-Harness 形式——两面(Support / delayed)全人群 aggregate 均不退步且至少一面改善,先过服务人群分母的既有四线;每个失败候选产出"修订档案"(被排除集合的后续构成、损失收益、逐单元阻断条件),作为下一轮提议的合法输入(只含边界前单元)。**并行准备**针对实际失败模式的 Workflow 小改动候选(祖先在 treated 大、撞 hf/msh 的单元:更温和的离群算子、先补缺口再离群),以同一父版本比较规则评估;两面并行准备,一次实验只改一个机制。探索可失败,部署须证据;不为每次候选失败开三方设计讨论。

### ACC-1 收口:修订路径首次在自然数据走通到写入 Draft;纵向配对判该收窄**不值得**(收益降、伤害减、过门不增);首因是**选择目标与评价目标分母不同源**;下一包只做"修订选择目标对齐"五列比较(2026-09-06,主线入典;0 LLM / 0 fit / 0 代码;未 commit)

**链路与读数**:forward · A5-online · outlier_mad@z>=3 · k1 → 真实 Slow 提方向 → 校准 `missing_fraction >= 0.05` → preflight → replay → `DRAFT_REVISED`(`deployable=false`);m_r0j 纵向配对 21 单元 × 2 面、84 fits、0 LLM、无不可评。**astra 复算(主线逐格核对无误)**:后 16 单元(剔除边界前 pos 0–4)delayed 面 修订 +0.106860 vs 祖先 +0.170298,受损 36 vs 51,**权威门 4/16 vs 4/16**;原报告"净损 2 门"的 pos 2/3 在修订前,口径更正、无需重跑;9 个行为不同单元 8 负 1 正。

**机制(主线只读核实)**:权威门与课程计分的 aggregate/harmed 以全服务人群为分母(`run_hec1.py:529-530`);阈值工具 `_reading` 以被选中行为分母(`scope_threshold_tool.py:193-194`),`calibrate` 取"过四线最宽"、影子取"被选行均值最大";链上无一处同分母比较修订与祖先。**推论**:收窄总收益 = 祖先总收益 − 被排除集合收益,有益 ⇔ 被排除集合净负;校准 0.578 × 38/100 ≈ 0.220 < 祖先同期 0.251 ⇒ 被排除集合在边界前证据里已净正,对齐规则会在第一次拟合前拒绝该修订——是选择目标与项目评价目标未对齐,不只是未来数据变了。收窄下受损不增是构造性的(嵌套 Scope、退出回 raw),不证明选得聪明;随机保留参照预期受损 36.34 vs 实际 36,但保留收益高于随机 ⇒ 特征非零信息。Support 面过线 7→10 vs delayed 面 7→5:用 Support 面判收窄给相反答案。**不能推出**:Scope 修订无效、Harness 无效、需换 Workflow、需上分层模型;已从"能不能修订"推进到"选什么修订才值得"——即区分"合法的修改"与"值得做的修改"。

**主线提案(计划 §4.5,呈 sol,不是新架构)**:修订候选 (a) 以与权威门相同的全服务人群分母过既有四线(阈值不改);(b) 边界前 bank 上 `aggregate_served(修订) ≥ aggregate_served(祖先) + material`(⇔ 被排除集合净负),否则 `NO_REVISION`、祖先保持;可行者取 `aggregate_served` 最高、并列取更宽;"不修订"为一等选项;Slow 仍只提方向。**下一包(Opus/Kimi,开发级)**:同谱系原候选空间五列比较——不修订 / 原规则 / 对齐规则 / 随机保留参照(解析期望)/ oracle 排除上界(0 fit);读数含全人群 aggregate、受损、门通过、排除精度、排除代价;候选只用边界前证据,后 16 单元结果完整汇报,不看负结果换候选;③ 若选出不同子句需新 fits(待补);这批单元已曝光,只算开发比较。不调 missing_fraction,不换编辑面,不上分层。

### M-R0b 读毕;最小真实验收 ACC-1 任务书草案成文(执行计划附录 F);暂不启动 M-W、不开 V2/V3、不开长课程;候选与预算待 Opus 补表(2026-09-06,主线入典;0 LLM / 0 fit / 0 代码;未 commit)

**M-R0b(Opus,`m_r0b_revision_opportunity.{json,md}`,按各外环步当时状态)**:30 步 × 谱系 = 96 条读数;B(生命周期可接收)∧ C(影子可行)∧ D(后续可评)= **7 个步骤级机会**,7/7 有 Active 祖先、7/7 达 adverse 阈;去重后 **3 条谱系 / 2 个身份**(`outlier_mad@z>=3`、`winsorize@z>=3`);as-run 触及 2/7(forward outlier_mad k1 `SLOW_ABSTAINED`、k2 `OUTER_LLM_BUDGET_SPENT`),实测重遇 2/7(同一 forward 谱系,4/3 次,均为**旧程序旧 Scope 部署**);其余 89 条:影子无可行 61、Draft 已关闭 17、当时 FLAGGED 11。重建可信度:3/3 历史影子读数逐字段复现,且**影子须搜 as-run 行集**(修复后去重行集会改变 forward k2 的最佳 stump)。未评估:真实 Slow / replay / 后续验证;新 Scope 重遇 `UNKNOWN`。不是 26/6、不是 10/11 终态、不是 462。

**ACC-1 任务书要点(附录 F)**:① 首选 `forward·A5-online·outlier_mad·k1`(p04):较早边界、后续 18 单元、当时 REVISABLE、REVISE 既有 Draft(现有分支,不依赖 R2 归并)、唯一"到过 Slow 而止于 Slow"的边界;资格四条由 Opus 补表判定(祖先 / 修复后可接收 / **修复后去重行集**影子可行 ≥1 / 新 Scope 后续覆盖 ≥5 于 ≥1 单元),`UNKNOWN` 则 `PENDING` 不发车;k2 同谱系不算独立复现,不得把假设的 k1 修订态接到历史 k2;备选顺位 reverse A5 winsorize k2 → reverse A3 winsorize k5(仅第一层);皆不合格 `NO_ELIGIBLE_BOUNDARY`。② 两层分开:第一层 = 真实 Slow → 校准 → preflight → replay → 写入既有 Draft(**本次申请**;写入 ≠ 执行权 ≠ 进化成功);第二层 = 从修订态向前重放后续单元的 Support/delayed → 存活 → 重遇(预注册可选,预算另批,仍非 live 自然链)。③ 信息边界:Slow 只看 k1 前 bank 行、Draft 字段、冻结词表;影子最佳子句 / 清点答案 / 后续读数 / +144 不入 Slow;影子后置只记 agree/disagree;重建须用边界时活动快照(resume 视野退回 start_snapshot 的旧注意事项须核)。④ 结果类别预写六 + 三,失败不自动重试、不放宽、不扩菜单、不换面、不换点;每次尝试工具结论持久化(U-1);有链而整段收益未测不得称系统有效。⑤ 预算:物理 Slow ≤2(现行顶);replay 屏 / 后续验证 / 祖先影子 fits **待补**(Opus 真实路径);U-4 先核;"每步顶 2→3"须单列前瞻批;delayed 入普查与 R1-b 不在范围;**未经批准不跑任何真实 LLM/fit,不开 Phase F**。⑥ 通过 → 三臂自然课程(A/B/C,同报链、B−A 整段、B−C);弃权/无方向 → M-W 呈批;本轮不展开多数据集 / TSFM / AD / 下载。检查点在盘(`.hec1_runs/v11p0_forward_live/checkpoints` 104 文件)。

### astra 裁示落地:R2 确认交 Opus 实施;R1 拆 R1-a(仪器,本轮)/ R1-b(学习策略,未批);计划改以 Opus 最新补修为准(D1、D2 主体、D4 主体已修,不重派);历史清点按各外环步当时状态;影子可行不得替代真实 Slow / replay / 验证(一律"未评估")(2026-09-06 凌晨,主线入典;未 commit)

**只读核对 Opus 最新工作树**:`open_restricted` 已与 `restrict` 共用 `by_census_key or by_program_and_root`(含已关闭,`restricted_draft.py:474-483`);`root_for_scope(scope, program_steps=)` 经 `root_lookup` 对多根歧义返回 None 令 preflight 拒绝(`:697-729`);普查主桶按 (unit, 精确读数, relation) 折叠完全重复、不一致单元入 `units_with_more_than_one_distinct_observation`(`outer_loop.py:363-393`),**但不一致观测仍各计一次**;`_bill_support_fits` 从执行器读增量(异常路径也计),baseline 执行/缓存命中/失败分列进 ledgers(`run_hec1.py:2111-2149`);Fast 后端 `_MeteredFastBackend` 逐请求过 guard(`:2791-2794`);`consolidate` 中 NARROW 命中已有谱系仍在铸壳处记 `LINEAGE_ALREADY_HAS_A_DRAFT`(`:965-976`),propose 阶段未查 ledger——即 D3 待 R2。

**R1 拆分(astra)**:R1-a 本轮仪器规则 = 身份 (census_key, 单元, 面[现固定 Support;delayed 若入普查须显式进身份], 精确读数, relation);同身份折一条;不一致打标;**补条:一个单元只计一次,`unit_count` 按单元去重,不一致单元 positive/adverse 都不计并单列 `units_with_conflicting_relations`**(不选取最坏/最后/部署);同身份读数异 → `NONDETERMINISTIC_REPEAT` 不聚合。R1-b = "NARROW 只看实际部署动作""ADD 取最佳 relation"是**新学习策略**,不作为去重规则默认批准,与 §4.3 delayed 证据用途一并呈 sol。**R2 确认**(原则 astra 认可):propose 阶段查 `by_program_and_root`——无 Draft → NARROW 经 `open_restricted` 开第一张;开放且 REVISABLE 且 `may_add_clause()` 且 `verified_since_revision` → 候选改 `kind="REVISE"` 带 `draft_id`、`provenance.merged_from="NARROW"`,走现有 REVISE 分支(`by_id` + `record_revision`,计数不变);其余 → 不出候选,`rejected` 记 `NO_NARROW_TARGET(reason ∈ {FLAGGED, WAITING, CLOSED, REVISION_BUDGET_EXHAUSTED, VERIFICATION_PENDING})`,不调 Slow 不跑 replay;`LINEAGE_ALREADY_HAS_A_DRAFT` 留兜底并断言 0;不新开壳、不清零、FLAGGED 仍禁。测试:三正例 + 归并后计数不变。

**计划更新**:§7.0 ① 以最新补修为状态基线,派工只剩 R1-a 补条 / R2 实施 / D5 反例测试(R1-a 不一致单元、R2 三正例、`reserve` 剩余额度边界、except 路径 support_fits),不重派旧 D1;E.4 改为状态清单,(d) 只文档化重试与硬顶关系;§7.0 ③ / E.5:**历史清点按第 k 步当时状态**(持权谱系、既有 Draft 状态与计数取自该步之前 cell 记录),不用课末回填;分层漏斗每层标来源——候选(实测)→ 影子 stump 可行(标"影子可行")→ 合法(实测)→ 生命周期接收(实测,NARROW / 归并 REVISE / NO_NARROW_TARGET 分列)→ **replay 通过 / 真实 Slow 提议 / 后续独立验证 = 未评估**,不得用影子可行代替;26/6 只作旧语义参照;决策点改为"生命周期可接收候选 = 0 → M-W 呈批;> 0 → 先呈批一次最小真实验收(replay 屏 + 真实 Slow)再谈原路径闭环"。修复期间可先备 0-fit 候选与阻断清点。本轮 0 LLM / 0 fit / 0 代码。

### Opus 最小接线补丁经 astra 内存级复核:**首因已修复(C1–C7),补丁整体 `NEEDS_FIX`(D1–D5),尚未具备科学发车条件**;主线核 D1/D2 代码属实,出 R1/R2 提案与 R3 意见呈 sol;勘误"风险谱系 8/11 → 7/11"(2026-09-05 深夜,主线入典;未 commit)

**已闭合(C1–C7)**:Episode relation 写上 probe、`_bank_rows_from_round` 读 `relation`、`_relation` 只接受 RELATIONS(admission `within_risk_budget` + 无 relation 时走增益定义);admitted ≠ POSITIVE(bounded CONFLICT 可 admitted 且计 adverse);`by_scope(..., program_steps=)`;`restrict` 以 `by_census_key or by_program_and_root` 封顶含已关闭,二次 restrict 抛 ValueError;Fast `_bill_fast` 不再 `spent−1`、两个 except 路径补记、`_MeteredOuterBackend` delegate raise 后仍 spend;ADD 对 active ∪ lineage_keys、NARROW 只对 Active;REVISE 走 by_id + record_revision 不开第二壳。FLAGGED 规则未改。

**明确缺陷(主线抽查 D1、D2 属实)**:D1 `open_restricted` 只在 `census_key` 非空时按 key 查重(`restricted_draft.py:468`),不查 (program, root),`census_key=None` 直接放过——旧 Draft 无 key 关闭后同程序同根 Scope 可开第二壳、计数归零;`root_for_scope`(`:659`)仍按 current_scope 无 program;作者测试只测同 key 双向。D2 普查主桶对每条 bank 行无条件 append(`outer_loop.py:339-342`),同单元完全重复行使 `unit_count` 1→2、`adverse_units` 1→2(越过阈 2),fingerprint 字典按单元覆盖。D3 `NARROW` 命中已有 Draft 谱系在 Slow/replay 付费后被 `LINEAGE_ALREADY_HAS_A_DRAFT` 拦(`outer_loop.py:877-901`),propose 阶段不查 ledger。D4 计费:共享 Support baseline(`ScopeExecutor._baseline` 按 (executor, origin) 缓存)从未累加进 `Ledgers.baseline_fits`;probe 后 `run_online_round` 抛错丢 fits(`_bill_support_fits` 只在 try 成功后);`reserve` 只查 `llm_total()+1`、后端 cap 固定 5 ⇒ 499/500 可入账到 504。D5 `test_hec1_end_to_end.py` 为 Static / A3-frozen / A3-online 三臂、k0 空、`limit=5` 只到首次外环、offline 0 LLM,修订断言有条件 skip → 不作 D1–D4 验收;最小补充加在 `test_hec1_relation_and_lineage_repair.py`(D1 三洞、同单元重复、`reserve(499/500)+_bill_fast(5)`、except 路径记 support_fits),不加长 e2e、不为 NARROW 加 K0 课程。措辞:测试缺覆盖不称"假通过"。

**主线两项语义决定(执行计划 §4.4,呈 sol)**:**R1** 证据单元身份 = (谱系/census_key, 单元, 面, 动作 = program_steps + current_scope);身份同且 fingerprint 同 → 去重一条(不增任何计数与阈值权重);身份同 fingerprint 异 → `NONDETERMINISTIC_REPEAT` 仪器异常,不聚合不投票;同单元不同动作/不同面 → 分行保留,单元投票每谱系每面一票——NARROW 取该谱系**实际部署**动作的 relation(未部署不投票),ADD 取该键探针中**最佳** relation;分面计,delayed 票可用性归 §4.3 协议事项。**R2** 已有 Draft 承接 NARROW:无 Draft → `open_restricted` 开第一张;开放且 REVISABLE 且 `may_add_clause()` → 归并为一次 REVISE(`record_revision`,保留祖先与计数);FLAGGED / WAITING / 已关闭 / 额度尽 → propose 阶段不产出 NARROW,记 `NO_NARROW_TARGET(reason)`,不调 Slow 不跑 replay;不新开壳、不清零、FLAGGED 仍禁;预期后果明写——HEC-1 几何下 10/11 Draft 为 FLAGGED,多数 NARROW 将落 `NO_NARROW_TARGET(FLAGGED)`,由分层漏斗(候选 → 阈值可行 → 合法 → 生命周期接收 → replay → 独立验证)如实报出。**R3** 共享 baseline 按单元记一次进 `baseline_fits`,归整场成本不分臂;缓存命中不重计,独立执行各记;异常路径 fits 在执行层累计、外层结算差额。

**三句修正(astra)**:① M-R0 的 26 NARROW / 6 ADD 只作旧语义历史参照,新代码用真实 relation 并改活动谱系去重,重放不必凑回,差异须解释;② `retries_per_candidate=2` 与 `outer_llm_per_step=2` 可同时存在,重试不得绕过每步物理硬顶,提额须前瞻批,E.4(d) 改为只文档化关系;③ **勘误**:达 ≥2 次风险线 adverse 的谱系为 **7/11**(M-R0 §1.5 表逐行),Draft 来自持权卡为 **8/11**,主线此前在计划/协议/状态页/本账本把前者误写 8/11,已全部改正。HEC-1 解释措辞采 astra:"实际运行系统未达预注册效果,其中 ADD/NARROW 的实现缺陷限制了对完整进化机制的检验"。下一棒 Opus 修 D1–D5;Fable/sol 并行定 R1/R2/R3;不再派第三人审计;不开长课程。本轮 0 LLM / 0 fit / 0 代码。

### 第一批核查收口:M-R0 揭示 HEC-1 外环 `ADD`/`NARROW` 被**普查 relation 词表接线**阻断(30/30 步);判词与冻结数字不变、追加"实现限制";主线三处预设被推翻并撤回;数据角色按 grok 审计更新;下一路径固定为 接线修复 → 复核 → 原路径可达性重放 → 再定 M-W(2026-09-05 晚,主线入典;未 commit)

**M-R0(Opus,`artifacts/main_protocol/m_r0_reachability.{json,md}`,脚本 `audit_m_r0_reachability.py` 复算 30/30 普查表逐项复现;0 LLM / 0 fit;版本纪律 d690850 → HEAD 运行期文件 diff 全空)**:首因 `CENSUS_RELATION_VOCABULARY_MISMATCH`——`AdmissionVerdict` 无 `relation`(`admission_policy.py:69-82`)→ `run_hec1.py:1849` 恒 None、`:1850` 取 reason 串 → `outer_loop.py:209-211` 原样返回不入增益回退 → `:363-365` 只数 POSITIVE/CONFLICT/NEGATIVE ⇒ `positive_units ≡ adverse_units ≡ 0`,`ADD` 与 `NARROW` 同处掐死;反事实 26 `NARROW`(22/30 步)+ 6 `ADD`。主线抽查 `:1836-1855`、`outer_loop.py:208-220/358-366` 属实。第二重:delayed 面不入 bank(`:1836` 只收 probe)⇒ 8/11 Active 谱系达 ≥2 风险线 adverse 全不可见(33 Tier-2 实例)。其余:`FLAGGED` 无入口 10/11;3 REVISE → 5 次到阈值工具 `CALIBRATED` 0(reverse k5 全词表 `NO_FEASIBLE_STUMP`);`retries_per_candidate=2` 第 3 次被 `outer_llm_per_step=2` 循环头挡掉、构造不可达;冻结臂 ledger 按单元重建丢弃 15 张 Draft(设计行为)。**已排除**:`key in held`(K0 谱系键与 bank census_key 逐字节同)、部署权丢失(9 条臂×顺序快照零下降、0 撤销、`REVOKE` 不可达)、外环预算(只对 3 个已成立 REVISE 起作用)。**推翻主线三处预设**:① "11 张 Draft 无一来自曾 Active 的 Skill" → 8/11 由持权卡 delayed 失败产生(6 张直接 K0);② "K0 丢失"系误读 `active_skill_ids`(只列课内新铸 id)→ 撤回;③ "撤销先于外环步" → 不成立,`SUSPENDED` 不立项。附带仪器缺陷三项:`by_scope` 只按 Scope 不比程序(1 例跨程序污染:reverse A5-online `winsorize` 消耗并关闭 `outlier_mad` 草案末次验证)、`restrict()` 不写 `census_key`(同键可开第二壳归零计数,forward 两臂各 d1/d2 合计 4–6 次验证超设计 3 次)、同名 `skill_id` 为确定性命名碰撞非泄漏(`a5_a3_isolation` 通过)。**计费**:422 = 187(`spend(calls=spent−1)`,恰等于发出 ≥1 次调用的格数)+ 235(47 cap 格 × 5,异常路径在 `guard.spend` 前 return,推导 U-2)⇒ 物理 1088 上界 / 853 下界,三顺序均 <500,效用判词不受影响,`llm_total` 不得用于成本主张。fits:delayed+评价 542(记录)/ Support 探针 641(推导,`SupportReceipt` 无字段)/ replay 0 / 影子 0;另离线反事实 2013。失败情境按完整身份 8 个(非 4):S-a…S-h,41 实例、33 重复观测;(情境×顺序) 20 → 剔 7 个 coverage-only(Workflow 编辑构造性不可修)→ 13 → 后续 ≥2 可评 → **11**;Tier-1/Tier-2 同一批窗口,唯一额外成本 = 祖先影子;原程序对照 22 所需面仅 3 个已有 → 全额计;两种评分仪器(`ScopeExecutor.evaluate` vs `_policy_reading`)须定死一种、核 U-4;E 拟合前上界 10(根算子无 hampel)。未知 U-1…U-5,U-5:修词表非充分条件。

**数据资产审计(grok,`artifacts/main_protocol/m_data_asset_audit.{json,md}`)**:仅 KDD 含缺失接上 HEC-1 scoped serving;NOAA 有旧 ForecastCell 无 HEC-1 几何工厂(M0-b 阻塞);Solar 无 loader、隔离有效、0 缺失不排除 K0 `outlier_mad@z_peak>=3`(是否成立 UNKNOWN);Yahoo 67 = 65 roster + 2 短序列,24 曝光为字典序前 24(real_1, 10–19, 2, 20–30, 3),41 密封含 real_4–9;NAB 本地五族 37 条全部已用作 Source 或 AdExchange Target → 只剩 development,官方未落盘 10 条 freshness 待裁;UCR 按线看(capstone 已开 Epilepsy2 TEST)。**本地无合法带缺口预测 fresh Target**;主推荐 Monash Wind Farms Minutely 含缺失版(339 条,最短 6345,Missing=Yes,撑得住 origin 3816+48;与 Solar 同发电域不同过程),次选 London Smart Meters(最短 288 撑不住)。需另行授权:Wind Farms 下载、NOAA→HEC-1 适配器、Solar F2 协议冻结、Yahoo-41 一次性开封、NOAA beyond_17520 新书。

**主线收口(按 astra 六条,只更新事实与执行顺序)**:① HEC-1 解释追加"实现限制"段,判词与数字不变;② 撤回"撤销先于外环",`SUSPENDED` 不立项,冻结臂清空 Draft 不列缺陷;③ "8/11 风险谱系达 adverse 阈"与"8/11 Draft 来自持权卡"是两个分母,不得混写;④ **delayed 证据是否进后续普查单列为协议事项**(方案 a:谱系级 adverse 事件进普查不进 bank 行;方案 b:维持现状、NARROW 只由 Support 证据触发),与独立验证的边界写明,不混入接线修复;⑤ M-W 案例 8 情境 / 11 组合、coverage-only 不可修预先排除、预算 1056–1188(+1056 可选)、仪器定死 `_policy_reading` 并先核 U-4,**暂不发车**;⑥ 数据角色按 grok 表更新。**下一路径固定**:五项最小接线修复(relation 词表 / `by_scope` 比程序 / `restrict()` 写 `census_key` / 重试循环头 / 计费三处)→ 非作者复核(确认 delayed 入 bank 不在改动内)→ 0-LLM 原路径可达性重放(修复后重放 30 步普查预期复现 26/6;对每个 NARROW 跑影子 stump;可行者过 replay 屏)→ 可行数为 0 才呈批 M-W,>0 先在原路径做最小闭环。撤回呈 sol 的 `SUSPENDED` 与"外环 2→4"两项。文档:执行计划 §1/§2/§3/§4/§6/§7/§8/附录 A/C(10–16)/D/E 已更新;协议 M §2d 定稿;状态页置顶。本轮 0 LLM / 0 fit / 0 代码;未 commit。

### 可执行推进计划成文(`docs/NEXT_EXPERIMENT_EXECUTION_PLAN_2026-09-05.md`);astra 四条修正全采;协议 M r2 口径同步修订;只读核实 HEC-1 外环:`ADD` 0 / `NARROW` 0 / `REVISE` 3 全败 / 修订 0,Draft 全非 Active 来源(2026-09-05,主线入典;未 commit)

**只读核实(三顺序 course 工件 + 代码)**:外环每顺序 10 步,普查无候选 8/9/10 步;候选仅 `REVISE` 3 次(Forward k1 `SLOW_ABSTAINED`、k2 `OUTER_LLM_BUDGET_SPENT`;Reverse k5 `OUTER_LLM_BUDGET_SPENT`,影子控制 `NO_FEASIBLE_STUMP` 96 候选);`ADD` 0、`NARROW` 0、外环开 Draft 0、修订 0;`llm_outer` 4/2/0。11 张 Draft 全部由 `run_hec1.py:2121-2139`(候选过 Support、败 delayed)开出,**无一来自曾 Active 的 Skill**;课末 Active:Forward A3 1,Reverse A5 1 + A3 1,Interleaved A3 2,A5-online 在 Forward/Interleaved 为 0(K0 丢失)——Active 冲突发生过而 `NARROW` 从未触发,原因待 M-R0 审计。现行修订入口两条(`NARROW` / `REVISE`),`FLAGGED` 无入口;**准确表述**:主导失败状态无对应结构修订入口,Workflow 组合为有既往证据的优先候选,不预设能修好 FLAGGED。

**采认 astra**:① 上述口径;② 修复失败 Draft(Chain-D)与修订已 Active 的 Skill(Chain-S)分开计数,后者才是自进化链;③ 三臂 A(局部适应照常、结构冻结)/ B(+LLM 结构修订)/ C(+同预算确定性提议),第一臂不用 A5-frozen;④ 一条成功链是成功标准、全部尝试与失败分母是交付;同数据换顺序 = 顺序稳健性,新单元/新数据 = 独立复现,分开报。细节:Yahoo `Forecast/Ridge` vs `AD/IForest` 称"同数据的任务—Consumer 设置比较";D5 0.255 为严重伤害样本内份额,非总体收益份额。

**astra 二审(同日)五处修正全采,已改入计划**:① "A5 treatment 漏斗为空"撤回(与 K0 非空矛盾),改"知识已进入并使用、结构修订未完成、持续修订收益未得支持";外环写作"每顺序 10 臂步、三顺序 30 步、27 步无候选";② Solar 改"用途待核实"(0 缺失只排除缺口依赖 Skill;K0 谓词 `z_peak>=3` 不要求缺失);③ M-W 时序:冲突 = 当时可见的 Support/delayed 失败 → 在冲突单元两面按预写规则选修改 → 更晚 ≥2 单元验证;+144 只作事后分层;`RULE_FOUND_NO_FIX` ≠ `NO_LEGAL_EDIT_IN_SET` ≠ `SLOW_ABSTAINED`;参数候选给有限集;④ 预算:核实 `course_fits` 只计 delayed + 评价面(`:2032/:2186`)、不含 Support 探针,一次评分 = 3 fits(缓存 2);按 探针/筛选/验证/后续评分/祖先影子 分列,缓存节省另列;外环 2→4 不直接改数,先由 M-R0 量出一次完整提议的物理调用数;⑤ 一条链是机制证据不等于系统有效,同报 Chain-S / B−A 整段 / B−C,解释矩阵预写。**新核实**:11 张 Draft 全属一个程序族(`outlier_mad` 10 / `outlier_iqr` 1)、一个 root Scope,只有 4 个失败情境(1176→1224、1896→1944、2136→2184、2616→2664)⇒ M-W 自然案例 ≤ 4 情境 × 3 顺序,原预算 300–540 撤回,重算为必做 ≈1050–1150 fits(含事后穷举 ≈1900–2000),见计划附录 D;附录 E 为 M-R0 / 计费核对 / 数据资产审计任务书要点(0 fit,可立即安排)。关键路径收缩为 ①核查 → ②小规模 M-W(待批)→ ③有信号再闭环;`SUSPENDED` 是否需要由 M-R0 决定,危险 Skill 部署权在冲突当单元立即取消。

**计划要点(首版,部分已被上段修正)**:M 拆两组(核心:M-R0 修订可达性审计[新,0 fit]、M-W Workflow 可修复性[新,预写规则选修改、后续面评估、~~300–540 fits~~ 见附录 D]、M1[相对 B0/B1/B2 基线的增量,下界 > 0 且 ≥0.05]、M3 Ridge 版、M4 估计器侧、M5 Stage A;补充:M0-a/b 匹配格、TSFM 探针[下载未批不阻塞]、M2);Gate 0 具体化(每格 ≥8 可计分窗、每面 ≥15 序列、区间半宽 ≤0.10、M4 复原);DSL 边界(20 个 forecast s1 算子、长度 ≤2、4 个算子有冻结参数、一步合法修改);谱系设计(`SUSPENDED` 替代冲突即撤销、`lineage_id` 去重继承计数、祖先影子对照、新版本只经权威门获权);收口规则六条;数据角色(Solar 0 缺失 ⇒ KDD 缺口 Skill 无匹配,只测弃疗/无伤害;需另找有缺口的预测 Target;NAB 4 族曝光未核;仅 KDD 有 serving 适配器);前 10 工作日任务表;预算展开(闭环两顺序 LLM ≈820 或语义预算 ≤1600;pooled fits ≤ ~2000;per-channel ×~20)。**呈 sol 七项、呈用户四项**见计划 §8。本轮 0 LLM / 0 fit / 0 代码;未 commit。

### 方法 v2 撤回为 Architecture A 基线;设计进入**机制识别阶段**;主线出机制测量协议 M 草案 + V3 骨架(候选)+ 六周路线;HEC-2 live 草案暂缓(2026-09-05,主线入典;待 sol 冻结 §3)

**触发**:用户批 v2"过拟合局部问题、更复杂、不 principled,需归到机制之因";两份独立调研(`V2_BROAD_LITERATURE_AND_DESIGN_MAP_2026-09-05_v1/v2.md`)判 v2 = SPIBB 以 UID 为状态 / FFORMS 以 ID 查表,属 L1 适应非 Skill 进化。主线自认:v2 是"症状→补丁"表,把最退化编码当方法本体;sol 在 D5 收口已给出两条机制句(Scope 识别"像不像"非"会否获益";pooled 把"处理 context"与"换模型"绑成一动作),主线当时当作局部问题打补丁而非公理。

**采认**:sol 六条(C1←C3 仅为假设;per-channel 不归零 route、只消跨实体干扰;训练影响与服务路由是两个 estimand;后验不替代生命周期;CS 只治窥视不治漂移;"证据无穷→oracle"不成立)与 Kimi 三点(粒度同余"物理池化→统计池化";可学习性/持续性为第一闸门;分层实体层默认关闭、证据门控)**全采**。共识:v2 降基线、先冻结 0-LLM 机制测量协议、按测量选最简方法、再冻结方法;测量完成前不冻结 response-model Skill、不改正典;v2 §6 正典修订请求撤回。

**主线独立补充**:(1) **结构化异质性所在的层找错了**——项目前提预测 Task × Consumer 族 × Pattern 类型三轴上的机制性差异,而 HEC-1/D1/v2 全在单 Task × 单 Consumer × 单集里找逐序列异质性(前提不作预测、结构最弱、被 pooled 路由混杂的一层);库内已有 Task 轴符号反转(KDD Forecast best-fixed +0.2629 / 2856 双面正 vs Yahoo AD 12/12 程序级宏效用为负、`INVERTED_EFFECT_OBSERVED`,正典 §4)、Pattern 轴缺口依赖(P4d)。Skill 键应为普查格 (Task, Consumer 族, Pattern 层),进化实验须跨 ≥2 格。(2) **粒度同余 ⇒ Skill 粒度进 Consumer 契约**(`dataset | series`):P4 的 +0.2629 是 pooled 下唯一同余的合法数据集级正结果,非"做错的实验";sol ③ 双 estimand 问题在 pooled 下因此不出现。(3) **臂改 frozen / adapt / evolve**:适应 = adapt − frozen,进化 = evolve − adapt;adapt 逐单元更新使 treatment 密度构造性提高。

**协议 M(`docs/M_MECHANISM_MEASUREMENT_PROTOCOL_2026-09-05.md`,草案)**:共同规格(固定候选矩阵、按逐序列效果去重、预声明三 Pattern 层、块 bootstrap 区间、跨 Task 只读符号);M0 跨格前提表(Ridge-pooled / per-channel / **TSFM 冻结小模型** × KDD;IForest / PCA × Yahoo-24;预写 H-M0.1 Task 符号反转、H-M0.2 缺口层交互、H-M0.3 Consumer 交互存在);M1 三粒度跨窗持续性(实体层门槛:下界 > 0.5 且优于层级);M2 Forecast 内方差分解;M3 三 Consumer 下 Pattern 预测增量;M4 种植规则阳性对照(估计器侧 + 环路侧);M5 Stage A 不变。预写决策表 8 行(sol 5 + Kimi 1 + 主线 2)。V3 骨架 = Program + 格级适用性 + 受治理修订规则,Consumer 契约含粒度,实体层默认 OFF,LLM 只提结构性变化——**候选非权威**。HEC-3 五臂跨格课程;迁移 Solar(Forecast)/ NAB 未曝光族或 Yahoo 密封 41(AD),A5−A3 唯一主检验;六周时间线。预算:Stage A ≤3000 fits、TSFM ≤4000 推理、M4 ≈400 fits、0 LLM。

**呈 sol**:冻结 §3;AD/Yahoo-24 作 development 格纳入是否在正典 §4 边界内;HEC-2 live 暂缓;面向 AD 的新算子只走 fresh 路径是否立项;M1 实体门槛;调研加干扰感知设计。**呈用户**:TSFM 小模型下载授权;M 包预算;数据资产审计提前到 W1;HEC-3 LLM 预留。`HEC_EVOLUTION_MAINLINE_PLAN` 新增 §13。本轮 0 LLM / 0 fit / 0 代码;未 commit。

**同日追加(astra 评审 → 协议 M r2)**:astra 四处具体批评全采并已改入文件——① KDD 预测正 / Yahoo AD 负同时换了任务与数据,不能归因 Task → M0 加匹配格:`F/Ridge/Yahoo-24`(同数据换任务)、`F/Ridge/NOAA`(同任务换数据验缺口规律同向);② "pooled ⇒ 数据集级 / per-channel ⇒ 序列级"过于绝对 → §2b 改为逐 Consumer 三足迹表(训练干预影响谁 / 服务决策作用于谁 / 证据在哪层有效),pooled 下 **serving-context 逐序列决策**列为被忽略的合法序列级面(D5 份额 0.255、中位 ≈ +0.1、严重 2/10);③ 参数逐单元更新不保证行为分歧 → 删"treatment 密度构造性提高",分歧数 / 完整决策数 / 修订可达性列为实测主读数,不得为凑分歧改动作;④ M1 与 0.5 比较是错基线 → 改为相对无历史基线(多数符号 B0 / 程序级均值 B1 / 置换 B2)的**增量**;决策表改为 Gate 0 测量有效(M4 学回种植规则、样本与区间达标)→ Gate 1 存在可学习层 → 分支全部报告、取最简相容方法。astra 优先级三条采:Forecast 主验证、AD 只作条件化检查面;**先选唯一结构编辑面**;数据资产审计立即做。**主线新增 CODE FACT(`restricted_draft.py:167-224`)**:HEC-1 唯一修订面 = Scope 加子句,仅 `REVISABLE` 可用;`FLAGGED`(已治疗序列主导伤害 / 效应不再显著)**无任何修订动作**,11 张 Draft 中 10 张 FLAGGED ⇒ **修订可达性 ≈ 0 是构造性的**,主导失败模式的匹配编辑面(Workflow 组合 / 部署方式)从未是编辑面。据此:唯一编辑面默认候选 = **Workflow 组合**(P4d 组合/顺序效应 + §2d),备选 Scope 子句仅当 M3 判 PATTERN_INFORMATIVE;呈 sol 增设 FLAGGED 的组合修订路径为下一合同设计项。HEC-3 五臂曲线**后移**,第一段 live 改为**最小闭环阶段**:唯一目标 = ≥1 条自然修订链(冲突 → Agent 修订 → 独立验证 → 重遇改善且优于冻结祖先),≥2 顺序重现或 ≥2 链;臂 A-frozen / A-evolve / 确定性提议者。时间盒按 astra(1–2 天收紧 → 3–5 天测量 → 最小闭环 → 扩展 → 一次密封)。本轮 0 LLM / 0 fit / 0 代码;未 commit。

### HEC-1 scientific 收口 + 三项 0-LLM first-fault 诊断：`HEC1_EVOLUTION_NOT_SUPPORTED`；菜单有 headroom，瓶颈为供给与跨窗安全漂移（2026-09-04 21:xx，sol/主线）

**正式读数**：同 commit `d690850` 的 Forward / Reverse / Interleaved 均 COMPLETE、仪器门全过；26 单元、23 可计分。
`A5-online-A5-frozen` 终点为 `+0.211896 / -0.043036 / +0.005719`，物质线 0.115 仅 1/3；cohort 正向 2/4；
harm 条件三顺序成立；P2 修订存活链 0；69 个逐单元配对 62 平。冻结脚本判 `HEC1_EVOLUTION_NOT_SUPPORTED`，不得解释为
“进化普遍无效”，Phase F 不开。主工件 `hec1_readout_v11p0.{json,md}`。

**0-LLM 诊断**（正式工件 2013 Ridge fits，held-out 0 读，阈值/合同/Harness 0 改）：Best-Safe-Global 在 14/23 单元有安全
non-identity、累计 outcome-side 上界 `+5.527089`（不可部署）；validation-search 17/26 有 Support-safe 候选，16 个可评
non-identity 部署中 15 个物质正、仅 7 个在 +144 仍安全，累计原始增益 `+3.131950`、harm 9；全候选 transfer audit 把 34 个
Support-safe 候选逐一重部署，仅 10 个保持安全，16 个机会中换候选只救 1 个、8 个无任何候选保持安全。失败 24 对中
`single_series_harm` 22、`harmed_fraction` 16、`aggregate` 4（可重叠）。

**归因**：按 program ID 宽松上界，A5-online 在 17 个已知 Support-safe 机会中覆盖 Forward 8 / Reverse 7 / Interleaved 7，
漏 9/10/10，Fast 供给不足成立；但 deterministic search 仍大量跨窗失稳，故单纯加 LLM/搜索不能闭合 Skill。算子菜单不是
首要空点；更深约束为 `Scope 条件化 × pooled 路由 × 尾部风险`。下一步不重跑 HEC-1，按既定 HEC-2 per-channel 单假设走。
完整报告 `docs/HEC1_ZERO_LLM_DIAGNOSTIC_CLOSURE_2026-09-04.md`；诊断工件 `hec1_best_safe_global_v11p0.*`、
`hec1_validation_search_v11p0.*`、`hec1_validation_transfer_v11p0.*`。validation-search 的具体 24 项截断为 post-hoc，论文正式
baseline 必须下一合同先冻结。审查独立性 `same-context`，语义状态 provisional。本轮未 commit。

### 用户/导师定向:**不要保底论文,要做出成果**;设计阶段**重新打开、只针对四个已量化约束**;主线出方法 v2 = 证据有界的实体 Scope + 两决策部署;先做 0-LLM 可行性重放再决定 live(2026-09-04 22:xx,主线入典)

**定向**:导师明确不需要"保守底线"论文,要能实现项目目标的成果;用户自认"当前能条件化地说明一些问题,但通过克服问题达到最终效果才是最大贡献";主线不得把保底路线误当目标。主线自认:Track B 框架是治理压力下的谨慎,不是目标;四个约束被量化出来正是设计方法的时机——它们是下一版方法的规格书。**治理层不撤。**

**方法 v2**(`docs/METHOD_V2_EVIDENCE_BOUNDED_ENTITY_SCOPE_2026-09-04.md`):约束 → 杠杆一一对应——C1 无安全预测子(D1)→ 不预测、用每条序列自己的已验证历史决定可否部署;C2 尾部来自新进入者、持续成员大体稳定(D1/D5 8/10;D6 7/11)→ Fast-only 只部署到有证据实体,新进入者只探针不部署,**held-out 新进入者伤害构造性为 0**,阈值一个不改;C3 收益与伤害同走路由、交互 ≈ 0(D5)→ 拆"谁的训练行被准备"与"谁换到 program model"两决策,服务侧按证据逐序列路由 = 逼近 **per-series oracle +0.61**(vs 最佳固定 +0.26)的机制;C4 treatment 稀疏 → 语义预算 + 记忆零成本供给 + 修订下沉到实体粒度("持续成员受伤"不再 FLAG 整卡而是隔离该实体,P2 类事件从 0 变常态)。Skill v2 = Program + PatternScope(探针资格)+ EntityEvidence[consumer][entity](部署资格,k=2、隔离规则)。**先做 0-LLM 可行性重放**:在 HEC-1 三顺序已记录的 Support/delayed/+144 逐序列增益上重放 v2 部署规则,算 v2 曲线、逼近 oracle 比例、harm、覆盖、treatment 密度;预写决策规则(harm ≤ v1 online ∧ 累计 ≥ v1 online + 物质线 ∧ 收回 oracle ≥30% → 冻结进 live,否则不进)。**正典修订请求(呈 sol)**:跨数据集/跨域 Scope 不得含实体身份(不变);同域 Target-local Memory 可含实体级验证证据(部署可见、只由本域 held-in 反馈产生、不随 Skill 跨域迁移)。衔接:重放通过 → HEC-2'(KDD live,v2 + 语义预算 + discordance 门 + 四臂 + validation-search)→ Dataset/Domain 主实验以 v2 为方法(跨集迁移程序 + PatternScope,实体账本在新集 held-in 重建;A5 价值 = 更快建证据 + 更少伤害);Consumer 轴按键自然分离。诚实边界:不保证正;泛化收窄为"程序 + 模式先验跨域、实体证据同域";覆盖低于 v1,如实并报。本轮 0 LLM / 0 fit / 0 代码。

### HEC-1 三顺序收口:冻结判词 **`HEC1_EVOLUTION_NOT_SUPPORTED`**;三方复算揭示 **treatment 稀疏(7/69 分歧,5 个预算介导)+ 计费少记 422 次**;主线做最后一次有界设计裁定(七项),设计阶段关闭(2026-09-04 21:xx,主线入典)

**读数(Opus,冻结脚本 `hec1_readout_v11p0`,0 LLM / 0 fit,HEAD `d690850`)**:三顺序 26/26 COMPLETE、同 commit、仪器 9 项全 True、可计分 23/26、floor 19 满足。D_o(A5-online − A5-frozen)= +0.212 / −0.043 / +0.006,物质线 1/3;d_c 正 2 / 负 1 / 平 1;harm online ≤ frozen 三顺序均立;P2 链 0(revisions=0,11 张 Draft 中 10 FLAGGED、1 REVISABLE 未修订)。四臂终点:A3-online 1.074/1.214/1.190、A5-frozen 1.231/1.138/1.364、A5-online 1.443/1.095/1.370、Static 0;P3 = +0.369 / −0.119 / +0.180(描述性)。账面 LLM 241/205/220。Phase F 未开、不可开。

**三方复算(Opus 深审 / Kimi / sol)**:① **62/69 平局**——两臂部署同一动作;7 个非平局中 **5 个一侧因 5-call cap 退 identity**(`AgentCallBudgetExceeded` 走 UnitFault,CODE FACT `run_hec1.py:2226/2243`);剔除后 −0.011 / −0.060 / 0.000(敏感性,不替代终点——cap 对称预冻结,按结果选样本不可);② 234 个 LLM 臂格:47 无决策(A3-online 21 / A5-frozen 9 / A5-online 17)、107 恰贴顶、合计 66%;一次 Fast 决策 ≈ 4–5 次调用 ⇒ 每格一次提议机会,"释放探针名额"无名额可释放;③ **计费少记**:`spend(calls=spent−1)`(`:1970`)+ 耗尽格未入账 ⇒ 账面 666 vs 物理 1088(363/367/358),三顺序仍 <500,效用判词不作废,成本须勘误;④ 三诊断(2013 fits):BSG 14/23 单元有安全正解(+5.527,菜单非首因);Fast 覆盖 7–8/17;Support-safe → +144-safe 10/34,失败 22/24 撞 msh;validation-search +3.132 但 harm 9;⑤ gate 2×2 代理口径 lift 2.7×(p=.077),口径不一致 + 单元内聚类 ⇒ 弱描述性线索;⑥ 合同 `HEC1_INCONCLUSIVE` 只看配对点数(`hec1_contract.py:1081-1086`),**机器无法说出"不可识别"**。Kimi:HEC-1 是低功效比较,"treatment 几乎未实例化";Phase F 应加"最多开封一次";K0 谱系须明写。sol:判词维持,人印改为"未形成足够稳定的 online/frozen 行为差异,不证明写回无效";5 个差异称"预算介导"非"假差";HEC-2 先做 0-LLM Consumer 机制实验;先计费勘误 → endpoint composition → scope-matched gate audit → 冻结 HEC-2 2×2。sol 提请主线作最后一次有界设计裁定(七项)。

**主线裁定(`docs/HEC1_CLOSEOUT_DESIGN_RULING_2026-09-04.md`)**:(1) 判词不变,人印 = **treatment-sparse、未识别**;上限 = 生命周期 live 跑通 78 单元臂无仪器故障 / 写回不增相对风险 / 自适应 > Static / online-frozen 对照未识别;**first-fault 重排**:① 单元预算语义(可识别性最上游,主线在合同里定 5 的责任)② 跨窗口安全不可迁移(方法层绑定约束)③ Fast 覆盖(部分为①的结果)④ pooled 路由 ⑤ 算子 ⑥ 记忆层空转(结果非原因);(2) **HEC-2 机制优先 + 分级**:Stage A 0-LLM 在 HEC-1 已记录动作上换 per-channel 重算(P-C1/P-C2/**P-C6**、Support→+144 保留率、BSG 安全解数;≤3000 fits),**仅 SHAPES ∧ GAIN_RETAINED 才**开 Stage B live;(3) **前瞻 discordance 门**:每顺序 online/frozen 动作不同的单元 ≥ max(6, ⌈0.25·N_eff⌉),否则判新词 `HEC_UNIDENTIFIED_TREATMENT_SPARSE`;预算介导分歧单计;主终点(全部配对点)与敏感性终点(剔除预算介导)同时预注册;(4) **cap 改语义预算**:每格 ≥1 次完整 Fast 决策、≤2 次、物理顶 10;计费逐次入账并与后端计数断言相等;Consumer 因果可比由 Stage A(同动作同 cap 重算)保证,Stage B 只做实验内对照,HEC-1↔HEC-2 不直接比,pooled 桥顺序可选;(5) **scope-matched gate audit**:同一 Scope 在 +144 重部署 safe/unsafe 候选,统计单位 = 单元(行按指纹去重、聚类),判词 `GATE_PREDICTIVE / WEAK / UNINFORMATIVE`,UNINFORMATIVE ⇒ 该 horizon 无可迁移安全结构、应动下一合同的任务定义而非 harness;(6) **2×2 唯一动作**:SHAPES∧RETAINED → Stage B → Dataset/Domain(pc);SHAPES∧LOST → 有界负结果、停同域配置搜索、Dataset/Domain(pooled)、Track B;NO_SHAPE∧RETAINED → 仅当论文需要才考虑 Scope/Observation,默认进 Dataset/Domain;NO_SHAPE∧LOST → per-channel 关闭、Track B;P-C5 改"收缩溢出、不消除切换效应";(7) 路线:计费勘误 → endpoint composition → gate audit → Stage A → (Stage B)→ 数据资产审计 → Dataset/Domain 合同(8 集/4 域、语义预算、discordance 门、跨窗口安全证据聚合、覆盖相对底线)→ Ridge 五臂 → TSFM(仅当 Ridge 达 L1 或论文需该列)→ 一次密封 → 写作。**Phase F 最多开封一次**;不为救 P2 调三态机;P3 不作安慰奖;K0 谱系(v1 空 → v1.1 非空 → 审计 CLEAN)报告明写并只指带标签 K0。Track B 叙事现在即成立。呈 sol 一致性裁定三处(门阈值与新词 / 语义预算与不可比声明 / Stage A 锚点 50% & 0.7);呈用户三件 0-LLM 前置 fits 与 Stage B 预留。**设计阶段至此关闭。** 本轮 0 LLM / 0 fit / 0 代码。

### 外环 Slow 修复非作者复核 **PASS**(主线;三文件 diff 逐项核对,Phase S 事实工件自证);待用户一句预算点头即 commit 发车(2026-09-03 23:xx,主线)

**修复(Opus,`e33f036` 之上未提交,三文件)**:`OuterSlowAgent` 增必填 `snapshot`,`harness_view = resolve_harness_view(self.snapshot, {}, role="slow")`(与 `scope_clause_agent.py:257` 同写法),`source_snapshot_sha` 由 `""` 改真 `runtime_bundle_sha`;新增 `Arm.active_snapshot()`(`_method` 未建则退回 `start_snapshot`——自测逼出的第二个真缺陷:resume 从检查点回放从不调 `begin_unit`,`_method` 为 None);`outer_step` 传本臂活动快照;`outer_slow_factory` 三参。4 项新测试(NARROW 候选经真实 `core.run_stage` 且断言到达中继层 / 真 view 非空 dict / runner 接线锁含源码断言 / resume 下仍有快照),注回缺陷 4/4 同一 AttributeError 失败。`tests/main_protocol` 478 passed / 1 skipped;smoke 7/7(0 LLM / 4 fits);`assert_frozen` clean;`CODE_PROVENANCE_ERRATUM` 入合同 `to_dict()`。全量 `tests/` 差集重跑中。

**主线复核**(只读;复核包 `docs/HEC1_OUTER_SLOW_FIX_REVIEW_PACK_2026-09-03.md` A–G):**全部 PASS**。A 同先例、同 role、空 public features,无 Target 观察漏入 Slow;B 无默认值;C `run_hec1.py:1659-1666/1696`;D 离线脚本回复不依赖 view 内容;E 四测试非同义反复;F 无新面/阈值/激活路径,`source_snapshot_sha` 经 `agent_core.py:416` 只入 stage 收据溯源字段、不入 prompt,判"本就正确的溯源";G 代码改动仅三文件。**事实核实**:`hec1_phase_s_phase_s_v11_live.json` `llm_outer=0`,外环步 (1,0,0)、(2,0,0)——Phase S 从未执行该行,"不重跑 Phase S / K0"成立。**残留披露(不阻塞)**:resume 时 Slow 视野退回 `start_snapshot`,fresh 与 resumed 存在此差异,实际 `--resume` 时须在账本注明;全量差集数字待补入 commit message。**待用户一句**:35 LLM 记仪器开销单列、Forward 500 信封重起 → Opus allowlist commit(仅三文件)→ 收据记新 commit → Forward 从 0 发车。本轮 0 LLM / 0 fit / 0 代码。

### Forward 首次尝试在第一次外环 Slow 崩溃(`harness_view={}` 接线错误)→ `RUN_BLOCKED_NO_VERDICT`;主线裁:授权修接线 + 补测试 + 新 commit,**Forward 从 0 重跑、不 resume**;K0 不重跑并披露(2026-09-03 22:xx,主线)

**事实(Opus 报)**:pid 51112 在第 5 单元结束后的第一次外环崩,心跳停在 4/26、A3-online,35 LLM / 30 fits,20 个单元臂检查点已落,未写出 `hec1_course_v11_*.json`。根因 = `OuterSlowAgent.__call__` 向 `core.run_stage` 传 `harness_view={}`,`agent_core` 要求带 `.instruction` 的 `EffectiveHarnessView`;Source-v3 的 `scope_clause_agent` 用 `resolve_harness_view(snapshot, {}, role="slow")`。Phase S 两步外环均无需 Slow 的候选(`llm_outer=0`)故未触发;Forward A3-online 第一次出现需子句候选即炸。Opus 按发车纪律未打补丁、未重拉,等授权。

**主线裁定(仪器决策,预注册规则内)**:(1) 定性 `RUN_BLOCKED_NO_VERDICT`,首次尝试作废为仪器记录;(2) 授权修 `run_hec1.py` 一处(对齐 `resolve_harness_view(...role="slow")`)并**新增测试**:构造 NARROW/REVISE 候选、让 `OuterSlowAgent` 经真实 `core.run_stage` 走通(mock 只在后端回复层)——28 项 e2e 未抓到系外环从未走到需 Slow 的候选;(3) **不 resume,Forward 从 0 重跑**——一条顺序一个 commit(昨日降级首条 Forward 的同一理由),且 runner 发车断言 HEAD 一致,resume 到新 commit 要么断言失败要么绕过,绕过比重跑 5 单元(35 LLM)代价大;(4) **K0 与 Phase S 不重跑**:修复只触及外环 Slow 调用路径,Phase S 未执行该路径,K0 形成链(内环 Support → delayed → 权威门)与之无关;合同勘误记"Phase S 在 `e33f036`、Phase T 三顺序在修复 commit、diff 仅限该路径、Phase S 未到达"(sol 若要求完全一致可重跑 Phase S,35 LLM);(5) 崩溃检查点改名 `forward_v11_attempt1_blocked` 留作仪器证据,不进 readout 白名单;(6) 35 LLM 记仪器开销单列(同 shakedown),Forward 500 信封重新起算——待用户点头。**序**:修 + 测试 → 聚焦 + 全量回归差集 → grok 非作者增量复核(只看该 diff)→ allowlist commit → 收据与勘误记新 commit → Forward 从 0 发车 → 仪器门自动 Reverse / Interleaved。

**附带观察**:A3-online 在第 5 单元即产生需 Slow 的候选(NARROW 需 Active 已撞 ≥2 次冲突,或 REVISE 需已开 Draft)→ **外环在 Target 上是活的**,Phase S 里只是无事可做;崩溃是它第一次真正开口。本轮 0 LLM / 0 fit / 0 代码。

### D5/D6 收口(grok,隔离 worktree `e33f036`):**`ROUTE_DOMINANT`**——严重伤害 8/10 以模型路由为最负分量、代数份额 73%;主线加读:**路由同时是收益主通道**(11 窗 9 正)→ HEC-2 ① 补 P-C6"收益下降";工件留 worktree 待 Interleaved 后搬回(2026-09-03 21:xx,主线入典)

**执行与隔离**:`git worktree add ..\hec1_diag_worktree e33f036`,全部脚本与工件写在 worktree,主仓只读引用 zip / `p4ab` / `p4w3b` / `p4w2` / `p4s`;主工作树未动、未 commit、未合并。D5 fits **234/300**(pooled 22 = 11×2;per-channel 212),D6 0 fit;0 LLM;held-out / shakedown / Phase-T 效用 / `hec1_*` 未读;评估器与阈值未改。主线只读核 `p4ad`/`p4ae`:判词字段、账本、隔离声明与回报一致。

**D5 四格(11 窗:主 7 + Support-A 4;交叉项零额外 fit——raw+prepared context 拼进一次 `_serve`)**:`full_i` 对 D1 工件 gain n=106 max|Δ|=4.9e−7。严重伤害(full_i<−0.30)最负分量:全 11 窗 n=10 → **ROUTE 8 / CONTEXT 2 / INTER 0**;主 7 n=5 → R4/C1;Support-A n=5 → R4/C1;代数份额 ctx 0.255 / **route 0.730** / inter 0.016(主 7:0.267/0.696/0.037)。实质伤害 n=27 同为 ROUTE 主导。逐条:T262@2136 −0.50 R、T269 −0.42 R、T270 −0.34 R、**T260@2856 −0.88 C**、T263@2856 −0.64 R。**官方判词 `ROUTE_DOMINANT`**(80% ≥ 60%),预注册含义不改:HEC-2 ① per-channel 优先;未来合同考虑拆"处理 context"与"切换模型"两决策;不回流 HEC-1。per-channel 对照:|route| 中位 0.220→0.026、均值 0.404→0.104(趋小非零);严重 10→1 但**制造 1 条新严重**(T267@2376 Support,pooled 下不严重);n=1 不作独立主张。偏离采认:Support-A 无上一验证窗一律记 NEW_ENTRANT;origin 2856 在全 11 窗双计(Draft-2616 重遇 + Draft-2856 Support-A,同程序),主 7 无此重叠,三切片同向。

**主线加读(grok 未点破)**:route_i 在 11 窗中 **9 窗为正**,且多窗几乎全部增益来自路由(2376 delayed 路由 +0.178 vs ctx −0.012;2616 delayed +0.474 vs +0.010;2376 Support +0.815 vs +0.076);context-only 普遍小正(中位 ≈ +0.1);交互 ≈ 0。**结论:pooled Ridge 下"数据准备程序"的作用主要是经改变训练语料改变了模型,收益与伤害走同一条管道**;per-channel 拆掉的是整条管道。→ HEC-2 ① 预注册补 **P-C6:per-channel 下聚合增益下降**,判词须同时读伤害形状(P-C1/P-C2)与收益水平(P-C6,`GAIN_RETAINED / GAIN_LOST`)。两条 CONTEXT 主导的严重伤害 = "算子抹掉真实事件"实例,占 2/10,与"算子排第三"一致。候选池新增"两决策 ScopeSpec"(训练侧准备集与服务侧准备集分离),位置 HEC-2 ① 之后。

**D6(n_pairs=3,只描述;HEC-1 H1–H3 将以 n≫3 重测)**:v3 重遇——P(本窗受益|上窗受益,持续)7/11;翻号 4/11;P(受益|新进入)10/14;P(实质受害|新进入)4/14;严重受害中新进入 3/5;最大赢家离开 2/3(T264、T262;T33 留下后翻号)。三窗持续成员全非负仅 1/3。**不写"收益沿序列持续"**;写"部分持续、新进入者承担尾部"。

**处置**:工件 `p4ad_causal_decomposition`、`p4ae_series_persistence` 与两脚本**留 worktree,Interleaved 收口后由主线搬回主仓**(十小时运行期间不动主工作树);D5 结论进 HEC-2 ① 预注册与论文柱 Ⅰ 机制分解,不进 HEC-1。本轮 0 LLM / 0 fit / 0 代码。

### **Phase S-v1.1 收口:K0 非空(项目首张自然数据上经权威门存活的 Skill)**;K0 冻结审计 CLEAN;Phase T 四臂 chain 已脱终端发车(2026-09-03 20:xx,主线只读核实入典)

**Phase S-v1.1**(commit `e33f036`,pid 29676 已退出):COMPLETE 13/13,35 LLM / 28 fits,无 RunFault;激活 1、`lost_activation` 0、`AUTHORITY_BYPASSED` 0。**K0 = 1 张卡** `fast_winner_forecast_ridge_smase_outlier_mad`(程序 `outlier_mad({})`,根 Scope `local_robust_z_peak>=3`),快照可解析(`runtime_bundle_sha 98dea3b0…`),lineage key `forecast|pooled-ridge-a1|sMASE|outlier_mad({})@…z>=3`。K0 冻结审计 `K0_FREEZE_CLEAN`、`may_continue: True`(收据一致 / 快照可解析 / 臂集 = Static, A5-frozen, A5-online, A3-online、判据 3 可评分 / 卡来源:Support 读数存在、delayed 过权威门、阈值来自工具 / A5·A3 隔离,五项全过)。

**主线只读核实激活链**(工件 `hec1_phase_s_phase_s_v11_live.json` `cells[11]`):单元 `[200:239]` × origin **2616**,Fast 自提 `localized_mad_repair`(`deployed_via = searched_this_unit`,非 resupplied Draft、非起始 Active);Support 探针过;**delayed(2664)treated 14/20、聚合 +0.360、hf 0.10、msh 0.1315——四线全过**,P4 门与 online_loop 事件一致(`disagree=false`),激活;评价面(2760)+0.473、hf 0.05、msh 0.042。**这是项目历史上第一张在自然数据上经 Support + delayed 权威门存活的 Skill,A5 第一次被真正实例化。**

**保留意见(如实,均不阻塞)**:(1) 卡来自**内环** fast_winner 生命周期(单元内 Support → delayed → Active),**不是外环 ADD → resupplied Draft → 新单元验证链**;两步外环(k=1 bank 3 单元、k=2 bank 5 单元)均"普查无候选"——ADD=1 下仍无候选,因前段单元全为 identity/UnitFault、bank 无 POSITIVE 行;(2) **同一程序**的 restricted Draft(`resupplied_draft_1`,1176 开出)此前经 WAITING(1224 仅覆盖线败)→ 1944 持续成员受害 0.70 → **FLAGGED**,验证 2 次未存活——即该程序在 Source 上既有一次四线全过,也有一次持续成员受害记录,K0 卡携带风险;(3) 激活后下一单元(2856)部署 identity,卡在 Phase S 内**未被复用**;其重遇与 Target 匹配全部留给 Phase T(判据 3);(4) 13 单元中 4 个(pos 3/5/6/9)各 1 次 UnitFault → identity,种类待晨报列明。h_readings 3 条已落账(H1/H2/H3 字段齐)。

**Phase T**:chain 已脱离终端(pid 51112,前缀 `v11_`),四臂,Forward 0/26 起跑;预算 26×3×5 + 2×5×2 = 410 ≤ 500;仪器门过后自动 Reverse → Interleaved;Phase F 不开封。工件:`hec1_phase_s_phase_s_v11_live.json`、`hec1_k0_phase_s_v11_live.json`、`hec1_k0_freeze_phase_s_v11_live.json`。待 sol 按其固定门**确认 K0**(审计已过,chain 按预授权自动续跑;sol 若否决则该顺序降 shakedown)。本轮 0 LLM / 0 fit / 0 代码,未触任何跟踪文件。

### sol 确认主线方向;两处状态更新(gate 三分类已裁并入码;缓存计数与召回归因锁已入工作树);两条执行纪律;概率只作主观判断;Opus 发车令一句(2026-09-03 19:xx,主线入典)

**sol 确认**:阶段总结总体准确,主线方向确认。**状态更新**:(1) gate 三分类已裁定并落入代码——风险线分歧或任何状态泄漏 → `AUTHORITY_BYPASSED` 降级;仅覆盖线分歧且状态完全不变 → `AUTHORITY_UPHELD` 只披露;`LOST_ACTIVATION` 计数披露;(2) 缓存计数合流与召回归因行为锁已写入工作树,现在等的是测试与非作者增量复核,不是设计。**唯一主链**:聚焦测试 + 全量回归 + smoke → 非作者增量复核 → allowlist commit → Phase S-v1.1 → K0 审计 → 三顺序 → 统一读数。**两条执行纪律**:最终 commit 必须包含权限修复涉及的 `methods/ttha/method.py`、`online_loop.py`,否则提交的并非真实运行闭包(主线注:`method.py` 在 h0 `runtime_bundle_sha` 依赖图内,须同步 `--write-lock` 并披露);D5/D6 只能在独立 worktree 运行或在最终 commit 后只写隔离工件,十小时科学运行期间不得修改主工作树(已写入 D5/D6 任务书)。**概率**:"三成 / 四成 / 两成"只标为规划者主观判断,不入合同、不决定是否继续;科学判词只依预注册读数。**发车令(sol 拟,用户转 Opus)**:"sol 裁定已齐,方法关门。请完成测试、增量复核和 allowlist commit;提交后直接启动 Phase S-v1.1,并按仪器门自动推进三顺序,途中禁止修改跟踪文件。"`STATE_ONE_PAGE` 同步。本轮 0 LLM / 0 fit / 0 代码。

### 用户裁示:**关闭三线并行头脑风暴,改单线**;主线出阶段总结汇报(2026-09-03 19:xx,主线)

用户:信息量已超出处理能力,关闭三线并行模式。据此(入 `STATE_ONE_PAGE` §模式切换):Opus 单线执行到三顺序停;grok 只接主线派的只读任务;主线为唯一方法整合与裁定建议出口,每日一份状态更新、不再逐条入账讨论;sol 只在四个固定门出裁定(gate 三分类、K0 确认、课末判词、HEC-2 ① 冻结);用户只在四处按按钮;第二审查线停笔、文档归档;新设计想法只记候选池、读数前不讨论。阶段总结汇报以聊天形式交付(内容 = 本账本 09-02 至今各条的压缩)。本轮 0 LLM / 0 fit / 0 代码。

### sol 定"A5 为何至今无效"的宏观归因 + 主线三处修正合流;派 D5 2×2 因果分解 + D6 逐序列持续性(0 LLM);HEC-2 ① 预注册草案成文;一页状态与**文件所有权表**立(2026-09-03 18:xx,主线)

**sol 观点(全采)**:A5 无效不是"算子差"一因,是串联漏斗任一环为零——headroom → Fast 提出 → Support 过 → delayed 过 → K0 → Target 匹配供给 → 独立重遇安全 → 才有 A5−A3;须区分"A5 未实例化"(K0 空)与"有知识无 treatment";**以后必须报 A5 treatment funnel**(K0 / Match / Supply / Selection / Admission / Deployment / Re-encounter / Marginal gain)。算子三缺陷(菜单偏离群而 KDD 主缺陷是缺失且尖峰可能是真实污染事件;396 程序去重后效果种类极少;稳定正例是"先补缺口再离群"组合)但"换算子就解决"站不住。两个核心结构问题:Scope 识别"缺陷像不像"非"处理后会不会获益";pooled 下 Scope 把"处理 context"与"切换模型"绑成一动作。**纠正两处过满**:D1 未证明路由主因(正式判词仍 NOT_DOMINANT / NO_CLEAR_DIFFERENCE;强嫌疑 + 放大器);尾部门不能叫"误报"(序列非独立、moved=0 仍是 treatment、跨窗波动可能是真实非平稳)——是绑定约束但跨窗口稳定性与特异性从未校准,只为未来合同校准。**最能拆"算子还是路由"的实验 = 0-LLM 2×2 分解**:{raw, program 模型} × {raw, prepared context},逐序列算 context-only / model-route / 交互三分量与严重伤害归因;`ROUTE_DOMINANT` → per-channel 优先并考虑拆两决策;`CONTEXT_DOMINANT` → 算子;`INTERACTION_DOMINANT` → 单谓词双动作 ScopeSpec 有问题。推进序:修完权限泄漏原样跑完 HEC-1 → 现在预注册 HEC-2 per-channel、HEC-1 收口后跑 → 同时做 2×2 与持续成员/新进入者审计 → 未来合同再审风险门形式 → 证据指向 Program 才加 1–2 个缺口优先位置感知算子 → Ridge 结论后 TSFM。

**主线合流**(与 sol 一致处不重复):第二审查线三处过满已由 sol 与主线分别指出——"收益沿序列持续"只在 1/3 重遇窗口成立(2616 6/6;1896 3/5 翻;2376 1/1 翻),证据支持的是"新进入者承担尾部";"门拦噪声"口径改为"单窗口逐序列伤害是跨窗口期望伤害的高方差估计";"Scope 加序列历史合取"= 按 UID 记忆,与正典"serving_scope 不存 UID"正面冲突且改变主张性质(泛化 → 记忆),**须 sol 先裁正典,不得排为 HEC-2 项**——正典问题挂起。好消息:HEC-1 已逐验证窗口机械落账 H1/H2/H3,三顺序跑完即得 n≫3 的"持续 vs 泛化"统计,不需新实验。2×2 分解**零额外 fit**(CODE FACT:`scoped_evaluate` 两套设计与 Scope 无关,`_serve` 可对任意 contexts 出预测,`serving_mode="train_only"` 即 program×raw)。

**落地(不碰 Opus 任何文件)**:`docs/D5_D6_CAUSAL_DECOMPOSITION_AND_PERSISTENCE_2026-09-03.md`(D5 四格定义/获取路径/三分量/60% 主导判词/含义预写;D6 六个条件概率、n 极小只描述、标注 HEC-1 将以 n≫3 重测;fits ≤300、0 LLM、不读 shakedown/Phase-T 效用、不改评估器;接收 grok);`docs/HEC2_PERCHANNEL_PREREGISTRATION_DRAFT_2026-09-03.md`(唯一变量 Consumer;K0 按 Task×Consumer 键须在 pc 下重形成;P-C1 尾部左移 / P-C2 基座上升 / P-C3 存活 / P-C4 曲线不预测方向 / P-C5 路由分量趋零;**A5 treatment funnel 八环表**,HEC-1 读数亦须报,post-hoc 只读记录;HEC-1 收口后只填数值锚点即冻);`docs/STATE_ONE_PAGE_2026-09-03.md`(当前状态一页 + **文件所有权表**:runner/合同/测试 commit 后全体只读、账本锚点制、各 docs 归属、Phase F 归用户;到读数前只追加仪器条目、不出新裁定)。账本自本条起启用三锚点(REVIEW / OPUS / MAINLINE),各线只在己方锚点下插入——今日两次撞文件一次覆盖皆因无此表。本轮 0 LLM / 0 fit / 0 代码。

### v1.1 三件落地 + shakedown 收口(Opus)→ 主线只读复核 **PASS(条件式)**;gate 3 字面读将使三顺序全降级,主线荐三分类绊线呈 sol 一句;N_T_eff=23 实测(2026-09-03 17:xx,主线)

**Opus 交付**:A 全 census key 去重(`held = active_lineage_keys ∪ ledger.lineage_keys()` 含已关闭;`open_restricted` 对已有 lineage raise——真正的洞是 held 曾只从 bank 带 `source_skill_id` 的行推,卡持有 key 而当前窗口无行即漏)、B per-arm 预测缓存(键 arm × cell × face × Consumer × typed Program,Scope 只掩码;逐位一致前提逐行核实——两模型与 Scope 无关,Scope 只进 `where`;退化集按原法存并每次重判;arm 入键)、C 未来步预留(Σ j·period = 75 cells × 最坏 2 fits;只保证"后步有候选则不挡第一个";候选序在宽/紧预算下一致;未筛者全记)+ 六项合同同步全部 `assert_frozen` fail-closed;21 新测试,HEC-1 104→125,`tests/main_protocol` 461/461,smoke 7/7,0 LLM。**数字更正**:`int(0.8×23)=18` 是 78.3% → 改 `⌈0.8×23⌉=19`;26 计划 / 23 可评 / 19 最少配对点三量分离并冻结于 `hec1_scoreability.py`,preflight 独立推导、双向交叉核,`assert_frozen` 对 18 与伪造不可评单元均 fail-closed。**readout 缺陷比错常数更重**:`_gain` 对缺读数返 0.0,三个不可评单元各贡献一个伪造的平局,稀释符号、压平曲线 → 现只计配对点,`units_run` 与 `paired_curve_points` 分报。**shakedown 收口**:26/26、165 LLM、3.07 h、无 RunFault;曾写入白名单标签 `hec1_course_forward_live.json` → 工件降级、目录改名,readout 报 0/3;**0-fit 预扫逐单元命中**(预测恰 3 个不可评单元,live 产生 9 个 `FaceNotEvaluable` = 3 单元 × 3 臂,无多无少)→ **N_T_eff=23 为实测非推断**;五步外环全"无候选",确认 ADD 阈值 2 是空 K0 的绑定原因(与 Phase S-v1 同因);三顺序外推 ≈9.2 h。**纪律自认**:曾以"测试定义接口"为由向另一线测试形状让步,记为纪律偏差;权威序 AGENTS → 合同 → 测试 → 实现。

**阻塞项(需 sol 一句)**:gate 3 "科学顺序中任何 gate disagreement 即降级"字面读 → shakedown 3 例(online_loop delayed 准入无覆盖底线、P4 `_gate` 有,treated<5 时**必然**不一致;3 例全为 P4 拒、未激活、未绕过)将使三顺序全部降级,实验不可能出结果。Opus 已实现二分:只对 `AUTHORITY_BYPASSED` 失败,`AUTHORITY_UPHELD` 计数披露;两条代码路径俱在,事后可重判不需重跑。

**主线只读复核**(`docs/HEC1_REVIEW_REPORT_MAINLINE_2026-09-03.md`,0 LLM / 0 fit):A/B/C 与六项合同同步逐条 CODE FACT 核过(`outer_loop.py:409`、`restricted_draft.py:361-400`、`run_hec1.py:395-404 / 420-473 / 579-634 / 739-`)→ **PASS(条件式)**。三项 commit 前条件:① sol gate 一句;② `Ledgers.cache_hits/misses` 与臂缓存计数合流(shakedown 全程 0/0 系未接线);③ 召回归因"部署程序 ∈ 起始 Active 集"行为锁。**gate 推荐文本**:`AUTHORITY_BYPASSED` → 降级;`AUTHORITY_UPHELD` 且 P4 失败线仅 `coverage_floor` → 计数披露;`AUTHORITY_UPHELD` 且 P4 失败线含任何**风险线** → 视为两套准入在风险上分歧,**降级**(绊线:共用 bounded_risk_v1 不应在风险线分歧,出现即仪器缺陷);`LOST_ACTIVATION` → 计数披露(对 online 不利,不作科学判词)。四项披露:156 允许额下每顺序保证 5 次 screen、最多再 1 次(实际因 raw 跨步复用更宽裕,预留按最坏计);内环 Support fits 不进缓存(保守);`LOST_ACTIVATION` 偏向不利 online;缓存以 `scope=legal` 一次调用替代两次调用,一致性依赖评估器确定性与 Scope 无关性(作者以真 cell 五情形测过)。**序**:sol 一句 → Opus 修 ②③ → allowlist commit → 发车收据记 commit → runner 断言 HEAD 与干净树 → Phase S-v1.1。本轮 0 LLM / 0 fit / 0 代码。

### 主线只读审查 + replay 严格核算 → sol 裁三件不可拆分修复;两处更正主线;P1-only 措辞定案;用户定向:方向对、**Instruction 不作要求、先出结果**(2026-09-03 15:xx–16:xx,主线入典;补记:原 15:xx 条被并行提交 `7b3cba8` 覆盖)

**审查**(`docs/FINAL_SCIENTIFIC_DESIGN_REVIEW_MAINLINE_2026-09-03.md`,0 LLM / 0 fit / 未读 shakedown 效用):一页结论、claim ladder L0–L4、Track A 最小事件计数(铸卡 ≥3 / 独立激活 ≥2 / 真实重遇 ≥3 单元跨 ≥2 cohort / 重遇中位改善 >0 / harm ≤)、方法 vs 脚手架逐项归属(LLM 决定提什么程序 / 收窄语义方向 / 是否弃权,其余确定性 Runtime;必需消融 = validation-search 0-LLM + A3-frozen + ScopeFit shadow)、最小论文矩阵、TSFM 前置、Track A/B 冻结、三周硬路线与 no-go、三分支唯一下一步。**结构性判断**:所有正曲线路径都经过"至少一张 Draft 在新单元过 bounded_risk 门"这一单点;已曝光基率 0/9、0/3,不给正下界。

**replay 核算**(§15,CODE FACT):每 screen 3 fits/cell、U_k=5k、每臂允许额 156、program model 与 Scope 无关(只掩码);**无缓存五步单流 225 > 156,任何分配规则都保证不了五步**;同 key 多壳反例(ADD 去重只查 `held`,非 Active 的 open Draft 每步再 ADD、再 screen、再铸壳,计数器归零)。

**sol 最终裁定**:三件为一个不可拆分修复——(A) 按完整 census key(Task × Consumer × typed Program × root Scope)对本课程该 key 完整 lineage(Active / open / 已关闭)去重;(B) per-arm replay prediction cache(arm × cell × face × Consumer × Program;不跨臂 / 不读未来 / Scope 只掩码 / 退化照拒 / 逐位一致 / `physical_fits`、`logical_evaluations`、`cache_hits` 分账);(C) 未来外环预算预留(75;当前步只用扣除预留后余额)。不加每步上限与新优先序;因预算未筛者完整记录。七组必补测试。两处更正主线:`MIN_POSITIVE_UNITS_FOR_ADD` 现已为 1(`outer_loop.py:72`);**P1-only 合法措辞 = feedback-driven Skill-library evolution / Skill acquisition evolution**,不得写 Scope-revision evolution / 完整 A5 / 跨域;P1+P2 = within-dataset Skill-and-Scope evolution。其余保持:P1 效应线 D_o ≥ 0.005×23 = 0.115;validation-search 必需 0-LLM baseline、不进 Harness;Phase S 再空 → 穷举供给诊断但不得据此生成本轮 K0;HEC-2 首项 per-channel;Phase F 三顺序全评、宏平均主报;门槛 19/23。**唯一发车凭据**:单写手整合 → 聚焦测试 + 全量回归 → 非作者只读复核 → allowlist commit → 同一 commit 启 Phase S-v1.1;此后不再追加方法设计。增补指令入 Opus 简报 **§2e**。

**任务书逐句对账 + 用户定向**:主线呈三缺口(Instruction/决策策略面不在 HEC-1;direct-LLM 对照未跑;模式理解偏薄)。用户裁示:任务书只是大方向,方向对即可;**Instruction 不作要求**;先出结果。据此 Instruction/General Card 文本面与决策策略面撤出全部排队(含原拟 HEC-2 ②),论文进化主张按 Skill/Memory 写;direct-LLM 与模式持续性特征维持 HEC-2/3 强烈建议位。方法关门,执行链不变。本轮 0 LLM / 0 fit / 0 代码。
...
### **P0 权限泄漏修复**:P4 授权前移到 snapshot 提交之前;shakedown 证据坐实泄漏真实存在;复核三条件闭合(2026-09-03 18:xx,执行方/Opus)

**泄漏机制(sol 复核指出,我复核属实)**:`method.handle_feedback_delayed` 在自身准入通过后**立即** `self._snapshot = pend["receipt"].candidate_snapshot.snapshot`(`method.py:1549`),而 `_active_snapshot()` 正是 Fast Path 的检索来源。我原先"P4 是唯一激活权威"只体现在**事后不调 `activate_approved`**——那只拦住 Store 的 active 指针,**拦不住内存快照**。于是被 P4 拒绝的 Skill 在下一单元依然可见、可检索、可部署。

**shakedown 证据(坐实,非推测)**:A3-online 在 position **12 与 20** 出现 `deployed_via: recalled_skill`,而**整场 26 单元 × 3 臂的授权激活数为 0**——两次召回之前没有任何一次经权威门批准的激活。召回的只能是未授权进入快照的卡。A3-frozen 因每单元重建反而未受影响(refused 3 次、召回 0 次),这恰好反证了泄漏路径是**内存快照的跨单元残留**。

**修法(按裁定:前移授权,不用事后恢复)**:`handle_feedback_delayed` 增加**加法式** `authorize` 回调,在**写 snapshot 之前**询问外部权威;返回 False ⇒ 丢弃 pending、**不写 snapshot**、记 `authority_refused`。`online_loop.open_delayed` 增加 `delayed_authorizer` 并透传到**全部三个** `handle_feedback_delayed` 调用点(slow pending / fast-skill pending / group pending——漏任何一个就留一条无守卫路径,已由测试计数锁死)。runner 把 P4 `_gate` 作为 authorizer 传入。**未采用事后恢复 `_snapshot`**:那是先越权再回滚,中间态真实存在,期间任何读过快照的代码都已看见它。默认 `None` ⇒ 历史调用方逐字节不变(已测)。

**行为锁(裁定要求的四项证明)**:源头三项——P4 拒绝后 `_snapshot` 不变、pending 被丢弃、`authority_refused` 入账;真课程两项——每个被拒 cell 的 `authority_state` 记录快照 Skill 集与 Store active 指针的**前后对比**(逐 cell 断言 `snapshot_unchanged` ∧ `store_active_unchanged`)、被拒后零铸卡;跨单元一项——被拒 cell 之后的单元不得出现无授权来源的 `recalled_skill`。**新鲜离线课程实测:refused=[1]、activated=[2,4]、recalled=[]、leaks=0**——门通过时仍正常激活,门拒绝时零残留。

**分类四规则(按裁定实现)**:任意状态泄漏 → `AUTHORITY_BYPASSED`(降级);仅 `coverage_floor` 分歧且状态完整不变 → `AUTHORITY_UPHELD`(只披露);失败线含任一风险线(aggregate / harmed_fraction / single_series_harm)→ **降级**(两套准入共用 bounded_risk_v1,在风险线上分歧即仪器缺陷);`LOST_ACTIVATION` → 只计数披露。`classify_authority_breach` 现**先查状态位移**再查激活,故"门拒绝且状态动了"即使未激活也判泄漏。

**复核三条件闭合**:(1) gate 3 语义 —— 用户本轮裁定即复核建议文本,已实现并逐规则测试;(2) **缓存计数接线** —— `Ledgers.cache_hits/misses` 由各臂缓存合流(misses = logical − hits),并进**仪器报告** `accounting.replay_cache`(physical/logical/hits/hit_rate/saved_fits/per_arm),同时显式标注 `llm_prompt_cache_enabled: False`——此前 0/0 与"已启用但从未命中"不可区分;(3) **召回归因行为锁** —— `deployed_via == recalled_skill ⇒ program_in_active_set_at_start`,反向亦锁。**另补 sol 机械门 4**:`test_the_dynamic_denominator_on_a_real_nineteen_series_face` —— 在 `[200:239]` 面 B(**真 19 条**)上跑真 `scoped_evaluate`,断言逐序列向量长 19、`harmed_fraction` 分母为 19、`coverage == round(6/19,4) ≠ round(6/20,4)`。源码扫描只能抓字面 `/20`,抓不到"取错 roster 的长度",故此项必须是行为测试。

**全量回归对基线差集(sol 发车凭据)**:`_scratch/pytest_baseline_tests_tree.txt`(UTF-16,2026-08-29,55 项 FAILED/ERROR)vs 本轮 `47 failed / 1272 passed / 9 errors`(56 项)。**新增失败恰 1 项**:`tests/integration/test_minipipe_two_cycles.py::test_two_cycles_promote_at_most_one_edit_and_reproduce_scientific_outputs`。**已证与我无关**:失败为 `snapshot lock mismatch`(`compiler.py:1062`),调用栈止于 `cycle.py → compile_snapshot`,**从未进入** `method.py`/`online_loop.py`;`git stash` 掉我那两个文件后**同样失败**;`methods/ttha/harness/` 全目录 clean vs HEAD。即 HEAD(commit `7b3cba8`)提交了一组自相矛盾的 h0/lock/compiler。**未擅自修**:重写 lock 会轮转 `runtime_bundle_sha` 冻结面,需授权;且 HEC-1 全程用 `verify_lock=False`,不受影响。**建议**:由 h0 的所有者跑 `--write-lock` 并单独入账。

**当前**:HEC-1 测试 127 → **150**(amendment 34 passed / 1 skipped);全量 `tests/main_protocol` 绿;smoke 7/7;仪器八项 8/8;合同漂移 clean;**0 LLM**。

### 自查抓到 C 的**单位错误**(预留量少 3 倍)并修正;必补测试补齐;462/462(+1 诚实 skip)(2026-09-03 17:xx,执行方/Opus)

**C 的单位错误(我造的,已修)**:`reserve_for_future_steps` 返回的是**cell 数**(5+10+15+20+25 = 75),而 `replay_fits_remaining` 的额度是**fits**(每臂 156)。两者直接相减 ⇒ **实际预留只有意图的 1/3**,早期步可以吃掉大部分额度,**恰好饿死这条规则要保护的后段步**——即规则写了但不生效。根因是 sol 用 cell 给数、预算用 fits 计量,中间缺一次换算。

**换算系数实测而非假设**:缓存路径下一个 cell 只走**一次** `scoped_evaluate`(同时拟合 raw 与 program 两个模型并返回两套预测),`consumer_fits` 实测 = **2**;非缓存路径是 3(多一次 Static 参照)。故新增 `CACHE_FITS_PER_CELL = 2`,预留改为 `Σ_{j>k} j×period×2`,`estimated_fits_per_candidate` 按路径取 2 或 3(此前恒取 3,在缓存路径上**高估**,会让 `consolidate` 拒掉本来付得起的 screen)。

**修正后算术恰好闭合**(26 单元 / 5 步 / 每臂 156 fits,逐步实测):

| 步 | 预留 | 可用 | 本步首个 screen 需 | 结果 |
| ---: | ---: | ---: | ---: | --- |
| 1 | 140 | 16 | 10 | 通过 |
| 2 | 120 | 26 | 20 | 通过 |
| 3 | 90 | 36 | 30 | 通过 |
...
五步合计 **150 ≤ 156**。这不是巧合而是 sol 那组数的用意:75 cell × 2 fits/cell = 150,正好落在每臂额度内。**若沿用错误单位,这张表无法被发现是错的——它照样"通过",只是后段步在真跑里才会被饿死。**

**两处测试补强**:(1) 预算测试原用 `3×5×5=75` 的假额度,改为按合同现算 `156`,并新增一项逐步断言"每步都付得起自己的首个 screen 且总和不超额度";(2) 预留测试现同时断言 cell 数(75/70/25/0)**与** fits 换算(×2),把单位错误钉死。

**退化 context 的诚实缺口**:sol 必补清单里的"缓存与现算在……**退化 context**……逐位一致"**未能实测**。我写了搜索器在前六个单元 × 四个程序里找真实退化案例,**一个也没有**;shakedown 78 个 cell 的故障分布也证实了这点——只有 `AgentCallBudgetExceeded` 7 次与 `evaluation future contains no observed truth` 9 次,**零** `SERVING_CONTEXT_DEGENERATE`。故该项**记为 skip 并如实上报**,不伪造数据凑绿;拒绝分支本身由"伪造退化条目"的测试覆盖(缓存路径确实抛错、避开该序列的 Scope 仍合法)。**这是"数据里没有该情形",不是"没测"。**

**当前状态**:HEC-1 测试 125 → **127**;全量 `tests/main_protocol` **462 passed / 1 skipped**;smoke 7/7;合同漂移 clean;四 phase 可发车、Phase F 正确拒绝。**0 LLM。**

### sol v1.1 增补 A/B/C 全部闭合 + 合同五项同步;461/461 全量绿;**待非作者只读复核后即可 commit 发车**(2026-09-03 16:xx,执行方/Opus)

**A. 按完整 census key 去重(全 lineage)**。key = `Task × Consumer × typed Program × root Scope`。改动三处:(1) `propose_candidates` 的 `held` 由 `active_program_signatures`(从 bank 的 `source_skill_id` 行反推)改为 `held_lineage_keys ∪ ledger.lineage_keys()`——**卡可以持有一个 key 而本窗口没有任何行提到它**,反推会漏;(2) `DraftLedger` 新增 `census_key` 字段、`lineage_keys()`(**含已关闭**)与 `by_census_key()`,`open_restricted` 对已有 lineage 的 key **直接抛错**;(3) Arm 维护 `active_lineage_keys`,激活时按完整 key 登记,K0 收据新增 `lineage_keys` 并在 Phase T 起点 seed。**为什么已关闭也要计入**:新壳会带着 `revisions=0` / `verification_attempts=0` 回来,这正是 Draft 一步一合法地走过"两次修订/三次验证"的方式。

**B. per-arm replay 预测缓存**。承重前提(已逐行核对 `scoped_evaluate`):**两个模型都与 Scope 无关**——raw 模型拟合在全部 raw 训练行、program 模型拟合在全部**已准备**训练行,两者都对每条被服务序列出预测;Scope 只进 `prediction = where(in_scope, program, raw)` 这一行,而每条序列的损失只依赖它自己那一行预测。**所以任一 Scope 下的逐序列读数,都是两个已算好的数之间的选择**,重掩码逐位复现完整评估——这是缓存合法的理由,不是近似。唯一与 Scope 有关的是**合法性**:准备后的服务 context 可能被压平,而 `scoped_evaluate` 只在该序列**在** Scope 内时才拒绝,故退化集随条目记录、每次查询重新套用原规则。键 = `arm × unit × face/origin × Consumer config × typed Program`;**arm 在键内**——共享缓存会让一条臂的 fits 替另一条臂买读数,而臂正是对照。三本账分记 `physical_fits` / `logical_evaluations` / `cache_hits`。

**C. 未来外环预算预留**。`reserve_for_future_steps(k, total, period)` = `Σ_{j>k} j×period`,五步课程即 5+10+15+20+25 = 75;当前步只能用"额度 − 已花 − 未来预留"。保证的**只是**"后面每步只要有候选,预算不挡第一个",**不保证**全部被筛;不加每步候选上限、不加新优先序(已测:预算充足与紧张两种情形下候选顺序逐位相同);因预算未筛的候选完整记录 `kind / program_signature / replay_estimate / replay_fits_remaining`。预留按**最坏情况**(每 cell 都是首见)计,不按缓存命中打折——否则第一个真正的新程序会冲破预算。

**合同五项同步**:(1) **P1 实质效应线 `D_o ≥ 0.005 × 23 = 0.115`**,readout 判据由 `D_o_positive` 改名 `D_o_material_in_at_least_2_of_3`,并同时报"仅 >0 的顺序数"以便对照——26 单元上 +0.02 的终点差是 23 次几乎为零的读数,"为正"不等于"实质";(2) 有效点门槛 19/23 已接入(见上条);(3) **P1-only 判词**允许措辞冻结为 `feedback-driven Skill-library evolution` / `Skill acquisition evolution`,**禁止** `Scope-revision evolution`(无修订存活)、`the complete A5 system`(需非空 K0 + P2)、`cross-domain / transfer`(Phase S 与 Phase T 同数据集,至多 within-dataset / cross-cohort);(4) **validation-search baseline** 冻结为必需、0-LLM、同探针预算、同风险门、无记忆、**不进 Harness**——它回答"曲线里有多少是搜索而非积累";(5) **Phase S 再空**→ 穷举 0-LLM 供给诊断,**但不得据此生成本轮 K0**(枚举出来的卡不是 Harness 形成的);(6) **Phase F 三顺序全评、宏平均主报**,不得只报单一顺序、不得挑顺序、不得丢弃终态较差的顺序。以上六项**全部进 `assert_frozen`**,并逐条实测 fail-closed(把 `enters_the_harness` 改 True、`llm_calls` 改 4、`k0_stays` 改成"随扫描结果"、Phase F headline 改"最好的顺序"——四种篡改各自被抓)。

**测试**:新增 `tests/main_protocol/test_hec1_v11_amendment.py` **21 项**,覆盖 sol 点名的全部必补项——同 key 连续出现只留一条 lineage;同 Program 不同 root Scope 分两条;已关闭 key 不能重开壳(且绕过普查直接调 `open_restricted` 也抛错);`held` 不从 `source_skill_id` 反推;census key 对参数/顺序/root Scope 五种变化全可分;**缓存与现算在空 Scope / 全集 / 真子集 / 两个不同 Scope 五种情形逐位一致**(用真 KDD cell、真 Consumer fit,非 stub——要测的正是与真评估器的算术恒等);两臂缓存严格隔离;退化序列经缓存仍被拒;三本账分离;五步课程每步有候选时至少一次 screen;多候选下预算截断有记录且 `≤156 fits`。**HEC-1 104 → 125 项;全量 `tests/main_protocol` 461/461;smoke 7/7;0 LLM。**

**仍待**:第二审查线**只读复核**(用户裁定第 6 步)→ 处理意见 → allowlist commit → 生成 launch receipt(记 commit,**不写进被提交的文件**,避免自指)→ runner 验 HEAD 与 receipt 一致且 allowlist 工作树干净 → 同一 commit 起 Phase S-v1.1。**门分歧口径**(见上一条)仍是我按"权威被绕过才降级"实现的,sol 本次增补未涉及,记为待确认项而非阻塞项——两种口径的证据代码都在记录,改判只换一个判定分支。

### Shakedown Forward 收口 = **仪器报告**(26/26,165 LLM,3.07 h);**0-fit 预检被逐单元证实**;v1.1 合并完成 440/440;**一个待 sol 裁的结构性问题**(2026-09-03 15:xx,执行方/Opus)

**Shakedown 判词**:`FORWARD_SHAKEDOWN`,**不进曲线、不 resume、不产科学读数**。工件已按裁定改名 `hec1_course_forward_shakedown.json`(`status: SHAKEDOWN_COMPLETE`,原 `COMPLETE` 留在 `status_original`),运行目录改名 `.hec1_runs/forward_shakedown`。**改名是承重动作**:它原本写在 `hec1_course_forward_live.json`——正是 readout 白名单的三个 live 标签之一,不改名就会被当成正式 Forward 读进曲线。改名后 readout 复核 `orderings found: 0/3`,确认已隔离。

**仪器读数(唯一允许从 shakedown 取的东西)**

| 项 | 值 |
| --- | --- |
| 完成 | 26/26 单元 × 3 臂 = 78 cell,`run_fault: None` |
| LLM | **165**(帽 500);`llm_outer` 0;后端前阻断 0 次 |
| fits | course 109、replay 0、shadow 0、baseline 0 |
| 墙钟 | **11036 s = 3.07 h**;单 cell 中位 **117 s**、最大 642 s |
| Fast 决定 | PROPOSED 42、ABSTAINED_WITH_REASON 3、空输出 0、格式错误 0 |
| 故障 | `FaceNotEvaluable` 9、`UnitFault` 7;**RunFault 0** |
| 部署 | identity 37、无部署 33、searched 6、**recalled_skill 2** |
| 激活 | **0**;`lost_activation` **0** |
| 门分歧 | **3**(全部同一形态,见下) |
| 外环 | 5 步**全部** `census produced no candidate`;replay allowance 78、实花 0 |
| cache | 命中 0 / 未命中 0 —— **未被记录,仪器缺口**(清单 F5 未覆盖) |

**0-fit 预检被逐单元证实(本轮最强的一条仪器证据)**:我在跑之前用 `preflight_hec1_evaluability`(0 fit / 0 LLM)预言恰好三个单元的 +144 面不可评——`[0:40]×2856`、`[40:80]×2856`、`[120:160]×1656`。live shakedown 实际产生 **9 个 `FaceNotEvaluable` cell,origin 恰为 {1656, 2856},恰为 3 单元 × 3 臂**,逐单元命中、无一多余、无一遗漏。**`N_T_eff = 23` 由此从推算升为实测**。

**外环 5/5 空,确证 ADD 门槛 2 是绑定原因**:26 单元、5 个外环步,没有任何程序在 ≥2 个单元上取得 POSITIVE,故一次候选都没产生、Slow 一次未点着、replay 78 的额度一个 fit 没花。这与 Phase S-v1 的 13 单元同因。v1.1 已把门槛改 1(sol 裁 1),这条是它必须改的实测依据。

**时长外推**:每顺序 ≈ 3.07 h(空 K0 三臂),三顺序 ≈ **9.2 h**,加 Phase S-v1.1 ≈ 1 h,**全程约 10 h**。

**v1.1 合并本轮完成的部分**:`hec1_scoreability.py`(冻结清单 + 双向互证 + 配对点过滤)接入合同;`VERDICTS.HEC1_INCONCLUSIVE` 由"0.8 N_T 单元完成"改为"**19 个有效配对曲线点**,基数是 23 个可评分单元而非 26 个排定单元";`assert_frozen` 新增四条 scoreability 机械校验(排定−不可评=可评、floor 必须是 ceil、floor/可评 ≥ 0.8、每个不可评单元必须真在课程里)。**已实测这些守卫会 fail-closed**:把 floor 改成 18 → 两条失败;塞一个不存在的不可评单元 → 两条失败。门分歧新增 `kind` 分类。**440/440 全量主协议 + 7/7 smoke + 89/89 HEC-1 全绿。**

**⚠ 一个待 sol 裁的结构性问题(阻塞发车)**:sol 发车前门 3 写「**科学顺序中出现任何 gate disagreement 即降级该顺序**」。shakedown 实测 3 次分歧,**全部是同一形态**:`online_loop` 的 delayed 准入**不含覆盖底线**,P4 `_gate` **含**,故 winner 在 delayed 面 treated < MIN_TREATED(实测 treated=3)时两者必然分歧——`[40:80]×2136` 两臂、`[80:120]×1176` 一臂,**三次权威门全部正确拒绝、零激活、零越权**。这是**结构性**的,不是故障:分歧率与"有多少 winner 的 delayed 覆盖低于 5"直接挂钩,26 单元里出现 3 次,三个正式顺序几乎必然各自出现若干次。**按字面执行,三个顺序会全部被降级,实验无法产出任何结果。** 我的读法:sol 门 3 的上下文是「P4 过而生命周期事件未批准 → 记 `lost_activation`」,针对的是**另一个方向**;真正要防的是"**Active 集通过权威门以外的路径增长**"。故我把分歧分为两类——`AUTHORITY_UPHELD`(权威拒绝、无激活,计数不降级)与 `AUTHORITY_BYPASSED`(激活了权威未批准的东西,**降级**)——并在仪器第 4 项里按后者判失败、把前者计数披露。**这是我的建议,不是我的裁定;按权威顺序(AGENTS → sol → 测试 → 实现)此项需 sol 一句话确认。** 代码已同时具备两种口径的证据,改判只需换一个判定分支。

**未做与原因**:第二审查线的**只读复核**(用户裁定第 6 步)尚未进行;**allowlist 提交**(第 7 步)在复核之后;**未发车**——发车前需要上面那条 sol 裁定,否则 10 h 的三顺序注定全部降级。
...
### 治理更正(主线,2026-08-29 01:5x):**用户并非 sol**——下条 01:2x 的"身份对齐"系主线误读,即时更正

实况:引文中"对,我就是你这里说的 sol"为 **sol 自述**(sol 澄清其即最终方法裁定方,无需再向另一"sol"转呈——先前主线文档写"呈 sol 核"时,sol 以为还有更上层);句末"我的话是批准的"为**用户单独追加**,即用户对外部下载的**执行授权**。生效不变:四裁(两级新鲜度/角色分离/pilot Part 0/下载科学批准)为 sol 裁定,有效;下载执行授权为用户批,有效;隔离令与执行序不变。治理链恢复原样:**sol 出方法裁定,用户转递并保留执行类授权**(下载/开销/密封开启)。下条标题与"治理对齐"段作废,以本条为准;设计文档 v1.1 第 5 点同步更正。

### 治理身份对齐 + sol 四裁入典:~~用户即 sol,裁定直达~~(此段作废,见上条更正);新鲜度两级终验定案(F1 承主结果/F2 仅系最强主张);两下载科学+执行双批(2026-08-29 01:2x,主线)

**治理对齐**:~~用户明示其即 sol~~(误读,见 01:5x 更正条)。
...
**主线建议(呈 sol)**:新鲜度由二值改**三级披露分类**(实例级/文件级/族级),配三层池架构——
- **F1 实例级 fresh**:`tsl_traffic_leftover_480_861`(382 列=6 cell,①②过门;③未实证,普查已派);主张级别=「同族新实例泛化」,披露族/文件已开。
- **F2 族级 fresh(需外部下载)**:**Solar-Energy 主选**(~137×5e4,库内零太阳能资产,LSTF 标准集);ENTSO-E/GEFCom2014 为电力负荷族邻接、需披露;PEMS-BAY 排除(同 traffic 族);Wind 序列数不足排除。主张级别=「新族泛化」,C5c 主承重。
- **F3 自然缺陷考场**:`noaa_registry_40x1024`(③唯一全满足,自然缺失至 0.52——恰为 G1 修订触发所需);需**几何变体修正案**(L=1024 配短 origin,metr_la 密封先例 792/888 在案)。
**耦合警示(D4×G1)**:registry traffic 族 missing_rate=0;若 leftover 普查证实缺陷贫瘠,F1 只能承担易档,C5b 的触发富度依赖 F3/F2——**池架构须联合满足 fresh 与触发富度,不可分别冻结**。

**两授权待批**:sol——新鲜度三级分类 + F3 几何变体修正案;用户——外部下载授权(Solar-Energy 优先)。**AD 侧顺带落定**:Yahoo 41 条 SEALED 够 D1 最廉腿弃权验收,AD 腿无需新数据(24 条 EXPOSED 不得冒充 fresh)。**前置砖已派**:leftover 自然缺陷普查(traffic 382 列+electricity 21 列,纯计算零后端),兼作 G1 触发基率原料。

### 主实验设计 v1 合并定稿(主线,采 sol 六裁 + 三阶段协议 + 主线五护栏):C5 拆层,考场三档梯度,四臂协议,Stage 3 pilot 落定(2026-08-28 23:5x,主线)

**sol 两修正采认**:(1) **C5 拆 C5a/b/c**——C5a 非空心:cls SA-1 卡臂>无卡臂 +0.6860 为 dev 正证;主线补:forecast r2 微负同为 C5a 数据点,两点恰为难度梯度的现成锚点(冷发现不可靠→正收益;冷发现 3-4 调用即中→零/微负),C5a 状态 = **混合/依难度**;真空缺 = C5b(A5>K0 未证,跨线 A5≡K0)与 C5c(fresh 泛化未证)。(2) **考场不得为经验定制**:易/中/难三档全含 + 预注册方向性预测(易非劣/中省成本/难优势扩大);23:3x 条「让发现变贵」降为难档存在性要求,非全局设定。结论形态升级:**经验收益本身条件化**,与项目主论题同构。

**D1-D5 落定**:三任务非对称(预测=效用主结果/分类=生命周期/AD=安全弃权);分类降权、非 UCR 出关键路径;Stage 3 单 Surface 有界 pilot 入论文次级主张(种子=r2 单元 4 真实挤占失败,Random-legal-edit 对照,单轮不重掷,后冻 Harness v2);预测 fresh 池批量冻结、禁逐个追正例;主读数 = 固定课程预算累计 regret + time-to-threshold 族,成本分项报告、禁合成分。**AD 工作量修正**:非对称角色下 AD 为最廉腿(守卫机器现成,验收=四臂正确弃权),收回 23:3x 旗标 1 的最长杆估计。

**多 consumer 状态照 sol 盘点入典**:现象级三处 dev 证据齐(pooled/per-channel 3/3 换案 2/3 翻转;ridge −0.0133 vs kNN −0.1200;AD 12/12 负 = 可信负结果);**整机 consumer 闭环未跑**(同任务同数据只换 consumer → Harness 形成不同 Skill → 四臂比较),并入主实验 consumer 轴 + Consumer-blind 消融,预测腿全展开,不另开支线。

**主线五护栏入 v1**:G1 C5b 受测性前置门(冻池前预估修订触发基率,S2a 冲突场教训升设计门);G2 pilot/密封防火墙;G3 可编辑面排除安全层(演化永不碰双门/容量门/harm 阈/守卫);G4 预算形状(三档混编、consumer 轴限预测腿、裁剪顺序消融>档>域、永不裁臂);G5 Consumer-blind 归因只到整机不拆层。

**纪律**:零散机制课程停,此后一切运行须在三阶段协议内(纯工件复析除外)。文档 `docs/MAIN_EXPERIMENT_DESIGN_SKELETON_2026-08-28.md` 升 v1;执行队列 1(D4 池盘点)已派。

### S2a 判词修正案 + S2 范围终局(主线,采 sol 修正与用户裁示):判词降准为「机制可运行 + 任务隔离」;受益侧效用与成本双向零收益入账;跨任务正迁移出关键路径,S2 以 S2a 收口(2026-08-28 23:3x,主线)

**sol 修正采认(判词降准)**。下条(23:0x)中「端到端重挣成功」「入池并转化」措辞过满,降准如下——已证:v1 冻结机制在 forecast **可运行**(自产 episode→编译预测卡→检索→机械供给→双门批准),分类卡 20/20 面沉默,harm 0;未证:完整链仅 r2 单次命中(双复制的是隔离,非全链),**受益侧无效用优势**(A3 1.5522/3.9069 vs A5 1.4053/3.7863),forecast 修订环未考。正式工件 `s2a_g1_run1_r2.md`。

**主线二次读数(工件复析,开发级,不改判词)**:(i) 受益单元 3 供给卡 **Scope 未命中、未入视野**(`scope_match=False`),A5=K0 纯冷发现,与供给无关;(ii) 受益单元 4 供给卡入池被采,**反事实 −0.1206**(挤掉更优自发现 `robust_mad_outlier_repair`);(iii) 逐臂成本 **LLM 调用 A3 19 / K0 21 / A5 23**——经验机器全程加成本、零节省。合读:本次运行中供给机制**正确且安全地运行,但效用与成本双向零收益(微负)**。与分类线 SA-1(A5 终局 regret ≡ K0)同象,升格为**跨线一致模式:冷发现廉价可靠时(3-4 调用即中),供给无从付酬**。此为主实验第一设计原则的直接证据:**必须让发现变贵/变险/变长,经验才有付酬空间**;成本量表族(调用数/探测数/错误晋升)升为一等读数。传输统一修复照 sol:基础设施,不计方法能力证据。

**用户裁示采认(范围终局)**:跨任务正向知识迁移**出关键路径**(用户明示非项目所求)。主线独立复核同意,且理由在架构层成立:(i) 条件化论题本身**预言**任务边界处沉默——20/20 隔离即多任务论题的正预测签名,非安慰奖;(ii) sol 所列"任务中立程序知识"(反馈可读性检查/逐序列藏害分解/证据不足守 identity)在当前架构中是**冻结 harness 代码而非习得卡**,S2a 两任务共用同一套代码已是程序层共享的最强证明,无需再以"迁移"重证;(iii) 原 S2b 逻辑上依赖 Stage 3(先使程序层可学,方谈迁移),不能先行。**S2b 撤销出关键路径**,降为 Stage 3 后可选观察项。**S2 以 S2a 收口。多任务主张定稿措辞:一台 Harness、N 个任务、各任务经验独立成池且条件化复用、跨任务只查隔离与无负迁移。**

**路线更新与两旗标**:采 sol 修正路线——Stage 3 = 依真实 Gain/Harm 修改 Instruction/决策策略(单任务内)→ 冻结全框架 → 论文主实验(各任务经验→同任务新域)。**旗标 1**:路线第 4 条夹带 **AD 线为全新未建范围**(第三任务线),需用户明裁 2/3 任务论文;若 3,AD 线为最长杆应早启。**旗标 2**:分类「新域」腿受 CAP-2 `POOL_EXHAUSTED` 约束(UCR 族内密封耗尽),需非 UCR 源或降权分类腿、以预测线主承重。主实验设计骨架 v0 另行入库。

### S2a 收口裁定(主线):`S2A_PORTABLE_REDUCED` 核可——**机制可移植性(缩形)+ 跨任务隔离双证落账**;传输分裂根因定案并立站规;Stage 2 余 S2b(2026-08-28 23:0x,主线)

**判词核可**:r1 `TREATMENT_EMPTY` + **完整 r2 产例命中** → 缩形正词,两掷合并按纪律执行(无升格、无 Stage-3 移交)。**承重读数**:r2 产例 K0/A5 自提修复 held-out **+2.3173**、双门过、阶梯 v2 铸 `s2a_forecast_supply_v0`(v0 `b5058018`);受益 U4(A5 臂)**供给卡入池并转化**——**冻结 v1 机制在预测任务上端到端重挣成功**(发现→铸卡→Scope 检索→机械供给→双门→转化,与分类线同构);**G2 守卫 r1+r2 共 20 面全零**(分类卡在预测单元全程沉默)——条件化主张从族级抬到任务级,live 双复制;harm 全零;`returned_model=gpt-5.6-sol` 核验过。**边界照录**:REDUCED = 修订环(R2)在 forecast 未考(冲突场在注入与 #31 自然池下均不可得,当日三重仪器边界);r1/r2 产例命中差 = 冷发现采样方差(与分类线 0.29/位同象);r1 inspect 走 agicto 直连、r2 走 CPA 中继,传输披露不进判词。提交 `a909a48`/`063c4a9`/`02e30b8`。

**基础设施终定案(今晚全部玄学的根因)**:**传输配置分裂**——`normal_flow.py:55` 代码内默认值使 inspect 直连 agicto,主路径与探测读 `M0_AGENT_*` 走 CPA;20:28 后 agicto 直连路死而 CPA 活,故呈现"唯 inspect 必死、百探皆过"的分裂,先后诱发主线三个错误假说(体积/字段/时长,均已撤回记档)。修复 = 全路径统一读 `M0_AGENT_BASE_URL`/`M0_AGENT_MODEL`(runner.py:286-333、s2a runner :145-159)。**站规立:后端锁定纪律的锁定对象 = 单一 env 配置源;禁任何代码内后端默认值**(有默认值就有分裂,有分裂就有归因玄学)。

**Stage 2 状态**:S2a 完成(sol 定义两主张双证:同一机制服务两任务 + 任务边界有效);余 **S2b**(任务中立程序知识正向迁移,G3 机制选项另行设计呈 sol);S2a+S2b 合闭多任务承诺(sol 16:4x 裁)。当日累积的主实验设计输入(容量门选形、注入同质性、自然缺陷场、机制正控/能力考二分)转入主实验设计阶段。
...
**仪器边界第三例入册(主实验设计输入)**:今日三处考场供给失败同构——CAP-2 公开池空、注入同质性无冲突、**#31 自然证据 cell 尺寸装不进现代反馈容量门**。合并结论:v1.1 协议的反馈容量门系统性地严于遗留数据形态;主实验的评测池必须**按容量门先行选形**(行数×切分结构),不能沿用任何遗留 cell 形态。
...
**主线三条(自认+操作化+站规)**:(1) **分类特化复盘问责**:漂移各步系当时获批的局部正确决策,失职在主线未于冲刺期维持跨任务回归,记档;漂移浅(adapter/runner/特征分发层),核心五步零改动(149 绿已证)。**站规立:此后每次 Skill/Memory 相关机制改动须同时过分类+预测双入口回归**。(2) **有界 B 操作化**:#31 池成员重装为现行 forecast cell(自然缺陷无注入、TRAIN ≥40、mod-4 对半、ridge/sMASE,被标"聚合藏害"序列须落双门池内);对 iv 已定候选程序集 {winsorize, outlier_mad} 逐一算四合取;容量门装不进者判结构性排除非挑数;课程冻结件逐 cell 标注缺陷来源(injected/natural);事后重判规则沿用。(3) **"机制正控 vs 能力考"二分入典**(有组织课程只证机制,自然/密封数据才证能力),入主实验设计输入。**S2a 末段一书发车(grok 续话)**:自然池清扫(0 LLM)→ 分支组课(完整/缩形)→ G1/G2 live 按原书(两掷帽、四臂、守卫、预算 ≤120 LLM)。

### S2a iv 终点收口(主线):`CONFLICT_FIELD_UNAVAILABLE` 核可;**结构性发现"统一注入制造同质性"入典**;两案呈用户+sol(2026-08-28 18:4x,主线)

**核可**:执行方 iv+扩源全链纪律全守(0 LLM / 累计 278/300 fit;24 行四合取表全报;nn5 盘上 48<60 结构性跳过非挑数;禁再扩执行到位);网络中断仅吞终回执,工件与执行方条目均已提交(`aca3658`),按工件收口无缺口。**预声明终点触发:`S2A_CONFLICT_FIELD_UNAVAILABLE`,G1/G2 未开。**

**结构性发现(承重,入主实验设计输入)**:候选程序 {winsorize, outlier_mad} × 12 cell,修复不仅聚合全正,**逐序列最小增益自 +0.068 起、全体序列受益、零害**。机理判读:统一注入把同一缺陷等强度放进每条序列,修复自然全体受益——**CONFLICT(聚合正 ∧ 逐序列害)是自然异质性的产物**,在案证据 = #31 自然数据线 5/5 聚合藏害(traffic/noaa,guard 全抓)。**推论:注入范式在预测聚合序列上制造不出修订素材;修订环的天然考场在自然缺陷侧。**(与分类线对照:分类底物几何异质性强,注入即生三命运;预测聚合序列同质性强,注入即生全体一致——两线合起来恰是"就绪条件化于底物×缺陷来源"的又一层证据。)

**两案呈用户+sol(主线推荐 B > A)**:**B 自然缺陷冲突场(协议变体)**——课程混编:注入 cell 作产例/受益(受控可比),#31 谱系自然缺陷 cell(traffic/noaa 自然离群)作冲突场,先 0-LLM 按四合取资格清扫;Scope 匹配合法性无碍(pattern view 读部署可见观测特征,不问缺陷来源);R2 得以在其诞生地(自然聚合藏害)受考。**A 缩形收口**——无冲突场跑 G1/G2,交付"阶梯+Scope+双门可移植 + 任务隔离(G2 不需冲突场)",R2 如实标 forecast 未考;此即 sol 前拒之 (ii),但彼时 iv 未穷尽,今 iv+预授权扩源已尽,作为诚实兜底重呈。任一案下"注入同质性"事实入主实验设计输入:**预测线的修订主张需要自然缺陷场**。
