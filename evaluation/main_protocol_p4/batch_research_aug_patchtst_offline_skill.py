"""DEV-AUG-PATCHTST-OFFLINE-SKILL (docs/DEV_AUG_PATCHTST_OFFLINE_SKILL_TASK_2026-09-24.md): the OFFLINE-SKILL design re-run with PatchTST as the
Consumer. Source evidence (12 scripted programs + real no-card zero-feedback Fast trajectories that see the PatchTST Consumer description) is
trained and scored with PatchTST; Slow forms two candidate domain cards per domain from that evidence only; Select picks one per domain by the
zero-feedback J; Test compares the PatchTST-learned card, the frozen MLP card, no card and a naive card regenerated for PatchTST (same Consumer
description and tool budget for every agent arm), with None / NoMix / the PatchTST-Source fixed program as references.

Reused read-only: the offline-skill zero-feedback Fast loop, adapter, metered client, Slow contract and statistics
(batch_research_aug_offline_skill), the official PatchTST model / setup / prediction of batch_research_aug_patchtst_transfer, the parent case
caches (materials by assignment key) and the transfer package's PatchTST Test fits (reused by material key, same frozen lr / steps).
New here: PatchTST description in the Fast / Slow / naive texts, a priority GPU queue with per-case batched fit workers, per-stage E barriers,
the PatchTST census, selection, readout and report. The parent packages are never written.
...
ROOT = REPO / '_scratch' / 'dev_aug_patchtst_offline_skill'
PARENT = OS.ROOT                                          # _scratch/dev_aug_offline_skill (read-only)
TRANSFER = PT.ROOT                                        # _scratch/dev_aug_patchtst_transfer (read-only)
MODULE = 'evaluation.main_protocol_p4.batch_research_aug_patchtst_offline_skill'
TASK = 'docs/DEV_AUG_PATCHTST_OFFLINE_SKILL_TASK_2026-09-24.md'
PACKAGE = 'DEV-AUG-PATCHTST-OFFLINE-SKILL'
IDENTITY = 'EXPOSED_DEVELOPMENT_REVALIDATION (the 40 Test cases were exposed by DEV-AUG-OFFLINE-SKILL and the PatchTST transfer; not an independent validation)'
DOMAINS = OS.DOMAINS
SEEDS = OS.SEEDS
...
HTTP = 12                                                 # user 2026-09-24: API parallelism 8-16
CPU_POOL = 2                                              # material builds (torch 1 thread, CPU)
GPU_LANES_DEFAULT = 1                                     # host free memory ~2.4 GB at planning; resources.json may raise it while running
MIN_FREE_GB = 1.0
EXTRA_LANE_FREE_GB = 1.6                                  # a second / third GPU lane takes a batch only above this free memory (worker peak ~1.2 GB)
MAX_BATCH = 36                                            # fits of one case per worker process (12 programs x 3 seeds)
SELECT_ARMS = ('d1', 'd2')
TEST_ARMS = ('f_patch', 'f_mlp', 'f0', 'f_naive')
...
# ============================================================================= the Consumer as the agents see it (task item 2: Fast sees PatchTST)
...
def consumer_p() -> dict:
    c = consumer_frozen()
    a = PT.ARCH
    return {'arch': 'PatchTST (official supervised model): each 192-point input window is split into patches of %d points with stride %d (end padding), '
                    '%d Transformer encoder layers, d_model %d, %d heads, feed-forward %d, dropout %.1f, RevIN instance normalization, one flat linear head '
                    'to 48 outputs' % (a['patch_len'], a['stride'], a['e_layers'], a['d_model'], a['n_heads'], a['d_ff'], a['dropout']),
            'input': 'univariate: every entity window is one sample of one channel; one model shared by the 16 entities of the case',
            'optimizer': 'AdamW', 'lr': c['lr'], 'weight_decay': 0.0001, 'n_updates': c['steps'], 'parent_batch': 64,
            'loss': ec.CONSUMER['loss'],
            'normalization': 'per-entity mean/std fitted on T only; derived pairs are NOT re-normalized; inside the model RevIN additionally normalizes each input '
                             'window by its own mean and standard deviation and restores them at the output',
            'metric': ec.CONSUMER['metric'],
            'training': 'shared PatchTST, AdamW, %d updates (training budget calibrated once on no-augmentation Source fits), parent batch 64 drawn over the '
                        'actual pool (entities x 433), the same T scaler and batch index stream for every plan of a case; dropout active in training only' % c['steps'],
            'serving': ec.CONSUMER['serving']}


FAST_SYSTEM_P = OS.FAST_SYSTEM_ZF.replace('one shared MLP Consumer', 'one shared PatchTST Consumer')
...
def freeze_plan() -> dict:
    ...
    plan = {'package': PACKAGE, 'task': TASK, 'frozen_local': now(), 'identity': IDENTITY, 'exposure_shown_to_fast': OS.EXPOSURE,
            ...
            'consumer': {**consumer_frozen(), 'architecture': PT.ARCH, 'official_commit': '204c21e',
                         'source': '_scratch/dev_aug_patchtst_transfer/consumer_frozen.json (None-only Source C_A+C_B calibration; not re-tuned; Test never used)'},
            ...
            'concurrency': {'http_in_flight': HTTP, 'trajectory_threads_per_stage': HTTP, 'cpu_material_pool': CPU_POOL, 'gpu_lanes': 'resources.json (default %d)' % GPU_LANES_DEFAULT,
                            'min_free_gb': MIN_FREE_GB, 'fit_batch': MAX_BATCH},
            'arms': {'source': ['f0'], 'select': list(SELECT_ARMS), 'test': list(TEST_ARMS),
                     'meaning': {'f_patch': 'domain card learned here from PatchTST evidence, chosen on Select', 'f_mlp': 'the frozen OFFLINE-SKILL domain card (learned with MLP evidence)',
                                 'f0': 'no card', 'f_naive': 'naive card regenerated with the PatchTST Consumer description (one call + at most one format correction, no selection)'},
                     'same_for_all_agent_arms': 'zero-feedback loop, tools, fields, per-trajectory caps, Fast system text and PatchTST Consumer description'},
            'slow': {'calls': '3 domains x 2 candidate domain cards (no shared card); census = PatchTST Source evidence only; MLP cards and MLP scores are not shown; '
                              'one format / contract correction; KEEP valid'},
            'selection': 'J_d(W) = mean over Select_d of seed-mean E(c, W) / E(c, None) on PatchTST; argmin; exact ties -> fewer deployment tokens -> slot order; '
                         'INCOMPLETE = None delivery',
            'fixed_source_p': 'per domain the scripted program (of 12) with the highest mean pp of None over the 8 Source cases on PatchTST; exact ties -> menu order',
            'refs_test': ['None', 'NoMix (P_NoMixRecipe)', 'Fixed_source_P'],
            ...
            'cache': 'materials by assignment key from the parent case caches (read-only); PatchTST Test fits of the transfer package reused by (case, material key, '
                     'seed) at the identical lr / steps (wiring re-fits one cached cell and must be bitwise equal)',
            'user_decisions_2026_09_24': ['budget caps frozen as listed', 'naive card regenerated with the PatchTST description', 'no added independent validation: '
                                          'the 40 exposed Test cases are a development re-validation', 'Test fits start when each commit freezes; E only after all',
                                          'API parallelism 8-16 (12 used)'],
            'transfer_results': 'reported separately (frozen MLP-derived materials on PatchTST); never counted as a Fast deployment of this package'}
    ...
    if not P_['resources'].exists():
        write(P_['resources'], {'gpu_lanes': GPU_LANES_DEFAULT, 'note': 'read by the running controller before every GPU dispatch; lanes share one RTX 4060'})
    return plan
...
# ============================================================================= transfer cache (PatchTST Test fits at the same lr / steps, keyed by material key)
_TCACHE = None
...
def transfer_cache() -> dict:
    ...
    if _TCACHE is None:
        ...
        cf = consumer_frozen()
        out = {}
        for c, mats in plan['material_bindings'].items():
            for m, b in mats.items():
                for s in SEEDS:
                    d = TRANSFER / 'test' / c / m / str(s)
                    if (d / 'fit.json').exists():
                        r = read(d / 'fit.json')
                        j = r['job']
                        if r['status'] == 'OK' and j['lr'] == cf['lr'] and j['steps'] == cf['steps'] and j['seed'] == s and (d / 'model.pt').exists():
                            out[(c, b['key'], s)] = str(d)
        _TCACHE = out
    return _TCACHE
...
def _fit_one(torch, train, j: dict, cfg: dict) -> None:
    """The transfer package's training loop (fit_job) with this package's paths; same model / optimizer / batch stream / loss."""
    ...
    with np.load(ROOT / 'common' / c / 'parents.npz') as z:
        ...
        yp = torch.as_tensor(z['yp'].copy(), device='cuda')
    child = None
    if j['material_path']:
    ...
    write(out / 'fit.json', {'status': 'OK', 'job': j, 'consumer': cfg, 'parameters': sum(p.numel() for p in model.parameters()), 'train_seconds': elapsed,
                             'peak_cuda_mb': torch.cuda.max_memory_allocated() / 2 ** 20, 'peak_rss_mb_process': _peak_rss_mb(), 'loss_curve': curve,
                             'torch': str(torch.__version__), 'device': torch.cuda.get_device_name(0), 'completed_local': now()})
...
def score_cells(spec_path: str) -> None:
    ...
    for cell in spec['cells']:
        model = PT.model_new(torch, int(cell['seed']))
        model.load_state_dict(torch.load(Path(cell['dir']) / 'model.pt', map_location='cuda', weights_only=True))
...
def gpu_lanes() -> int:
    try:
        return max(1, min(3, int(read(paths()['resources'])['gpu_lanes'])))
    except Exception:  # noqa: BLE001
...
class GPUQueue:
    ...
    def start(self, lanes_max: int = 3):
...
# ============================================================================= zero-feedback Fast with the PatchTST description
...
def run_branch_p(bdir: Path, case: str, knowledge: br.Knowledge, led, client, *, cs: ec.CaseSpec, arm: str, resume: bool = False) -> dict:
    """OS.run_branch with the PatchTST case card and Fast system text (everything else identical)."""
...
def run_fast_stage(stage: str, jobs: list, knowledge_of, *, led, client, on_commit, sdir: Path) -> dict:
    ...
    summ = {'stage': stage, 'finished_local': now(), 'n_jobs': len(jobs), 'complete': sum(r.get('status') == 'COMPLETE' for r in results.values()),
            ...
            'missing': ['%s__%s' % j for j in missing], 'errors': errors, 'unknown_usage_blocking': rt.unknown_usage_blocks(led)}
    ...
    if missing or summ['unknown_usage_blocking']:
        raise RuntimeError('%s stage incomplete: missing %s errors %s' % (stage, summ['missing'][:5], errors[:3]))
    return summ
...
def make_client(led, stage: str):
    return OS.ZFMeteredClient(led, ROOT / stage / 'llm', http_cap=CAPS['http'], stage=stage, incidents=paths()['incidents'], extra_cap=CAPS['extra_transport'])


# ============================================================================= Source evidence on PatchTST
...
def selection(plan) -> dict:
    ...
    for d in DOMAINS:
        ...
        if not cands:
            rec['domain'][d] = {'arm': None, 'slot': None, 'skill': None, 'note': 'no usable domain card; f_patch runs without a card'}
            continue
    ...
    with DP.PackageLock(ROOT, 'run'):
        ...
        def source_pipeline():
            try:
                ...
                fx = fixed_source_p()
                test_refs_ready.set()
                print('FIXED_SOURCE_P', json.dumps({d: v['label'] for d, v in fx['domains'].items()}), flush=True)
                st.set('slow', 'running')
                ...
                st.set('test_patch', 'fast')
                run_fast_stage('test_patch', test_jobs(plan, ('f_patch',)), knowledge_test, led=led, client=make_client(led, 'test'), on_commit=on_test_commit, sdir=P_['test'])
                st.set('test_patch', 'done')
            except Exception as exc:  # noqa: BLE001
        ...
        def fixed_ref_pipeline():
            try:
                ...
                for c in [c for d in DOMAINS for c in sorted(plan['cases']['test'][d])]:
                    ...
                    on_test_commit(c, 'Fixed_source_P', {'physical_material': refs[c]})
...
PAIRS = (('patch_minus_mlp', 'f_patch', 'f_mlp'), ('patch_minus_f0', 'f_patch', 'f0'), ('patch_minus_naive', 'f_patch', 'f_naive'),
         ('mlp_minus_f0', 'f_mlp', 'f0'), ('naive_minus_f0', 'f_naive', 'f0'))
REFS = ('None', 'NoMix', 'Fixed_source_P')
...
def readout() -> dict:
    ...
    for case in [c for d in DOMAINS for c in sorted(plan['cases']['test'][d])]:
        ...
        rp = {'None': 'None', 'NoMix': 'P_NoMixRecipe', 'Fixed_source_P': fixed['phys'][case]}
        den = em['None']
    ...
    # the transfer package (frozen MLP-derived materials; not a Fast deployment of this package)
    tr = read(TRANSFER / 'result.json')
    tplan = read(TRANSFER / 'plan.json')
    same_mlp_delivery = sum(1 for c, r in per_case.items() if registry(c)[r['delivered_phys']['f_mlp']]['key'] == tplan['material_bindings'][c][tplan['choices'][c]['F_domain']]['key'])
    same_f0_delivery = sum(1 for c, r in per_case.items() if registry(c)[r['delivered_phys']['f0']]['key'] == tplan['material_bindings'][c][tplan['choices'][c]['F0']]['key'])
    res = {'package': PACKAGE, 'identity': IDENTITY, 'written_local': now(), 'cards': {d: fr['domain'][d]['arm'] for d in DOMAINS},
           ...
           'transfer_package': {'table': tr['table'], 'note': 'frozen MLP-trajectory deliveries trained on PatchTST (previous package); not a Fast deployment of this package',
                                'f_mlp_same_delivery_as_transferred_F_domain': same_mlp_delivery, 'f0_same_delivery_as_transferred_F0': same_f0_delivery},
...
def source_evidence_summary() -> dict:
    """PatchTST vs MLP (screen) scripted-program means per domain on the same 8 Source cases, and the no-card Source commits under each description."""
    plan = read(paths()['plan'])
    out = {}
    for d in DOMAINS:
        cases = sorted(plan['cases']['source'][d])
        ...
        out[d] = {'patchtst_G_by_program': pt, 'mlp_G_by_program': ml, 'patchtst_best': max(pt, key=pt.get), 'mlp_best': max(ml, key=ml.get),
                  'no_card_commits_patchtst_description': dict(OS.collections_counter(commits_p)), 'no_card_commits_mlp_description': dict(OS.collections_counter(commits_m)),
                  'no_card_G_patchtst': f0p}
    return out
...
def smoke() -> dict:
    ...
    plan = freeze_plan()
    check('fast_text_patchtst', 'PatchTST' in FAST_SYSTEM_P and 'MLP' not in FAST_SYSTEM_P)
    check('slow_text_no_mlp', 'MLP' not in slow_system_p('D01') and 'screen' not in SLOW_EVIDENCE_P.lower())
    cp = consumer_p()
    check('consumer_desc', 'PatchTST' in cp['arch'] and cp['n_updates'] == 500 and 'MLP' not in json.dumps(cp))
    prepare_all(plan['cases'])
    cc = css()
    bad = []
    ...
    tp = TRANSFER / 'cases' / 'D03_Q_Q1_G1' / 'parents.npz'
    with np.load(tp) as x, np.load(paths()['common'] / 'D03_Q_Q1_G1' / 'parents.npz') as y:
        check('parents_equal_transfer', np.array_equal(x['Xp'], y['Xp']) and np.array_equal(x['yp'], y['yp']))
    check('transfer_cache_loaded', len(transfer_cache()) == 567, n=len(transfer_cache()))
    # scripted zero-feedback plumbing on one Source case in a scratch dir (no real client object)
    ...
    check('fast_saw_patchtst', seen.get('system') == FAST_SYSTEM_P and 'PatchTST' in txt and 'MLP 192' not in txt)
    ...
    # the MLP card still loads; knowledge objects for every Test arm exist without any Test result
...
def wiring() -> dict:
    ...
    reg = registry(src)
    tcase, tmat, seed = 'D03_Q_Q1_G1', 'P_NoMixRecipe', SEEDS[0]
    treg = registry(tcase)
    j1 = {'cid': ec.cell_id(src, 'C_shock', seed), 'case': src, 'phys': 'C_shock', 'seed': seed, 'key': reg['C_shock']['key'], 'material_path': reg['C_shock']['path'],
          'stage': 'source', 'output': str((P_['fits'] / src / 'C_shock' / str(seed)).relative_to(REPO)).replace('\\', '/')}
    j2 = {'cid': 'wiring_refit_' + ec.cell_id(tcase, tmat, seed), 'case': tcase, 'phys': tmat, 'seed': seed, 'key': treg[tmat]['key'], 'material_path': treg[tmat]['path'],
          'stage': 'wiring', 'output': str((wd / 'refit').relative_to(REPO)).replace('\\', '/')}
    for j in (j1, j2):
    ...
    t0 = time.time()
    rc = run_gpu_process(['--fit-batch', spec], P_['logs'] / 'wiring_fit.log', 600)
    secs = time.time() - t0
    for j in (j1, j2):
    ...
    tdir = transfer_cache()[(tcase, treg[tmat]['key'], seed)]
    ...
    write(sspec, {'stage': 'wiring', 'case': tcase, 'cells': [{'cid': 'refit', 'dir': str(REPO / j2['output']), 'phys': tmat, 'seed': seed, 'key': treg[tmat]['key']},
                                                           {'cid': 'transfer', 'dir': tdir, 'phys': tmat, 'seed': seed, 'key': treg[tmat]['key']}], 'out': str(wd / 'score.json')})
    rc2 = run_gpu_process(['--score-cells', sspec], P_['logs'] / 'wiring_score.log', 600)
    sc = read(wd / 'score.json')['cells']
    tsc = read(TRANSFER / 'scores' / (tcase + '.json'))['scores'][tmat][str(seed)]['nmse']
    f1, f2 = read(REPO / j1['output'] / 'fit.json'), read(REPO / j2['output'] / 'fit.json')
    res = {'written_local': now(), 'fit_rc': rc, 'score_rc': rc2, 'second_fit_in_same_process_bitwise_equal_transfer_model': bitwise,
           'e_refit': sc['refit']['nmse'], 'e_transfer_model_rescored': sc['transfer']['nmse'], 'e_transfer_stored': tsc,
           'e_equal': sc['refit']['nmse'] == tsc == sc['transfer']['nmse'], 'train_seconds': [f1['train_seconds'], f2['train_seconds']],
           'batch_wall_seconds': round(secs, 1), 'peak_rss_mb_process': f2.get('peak_rss_mb_process'), 'peak_cuda_mb': max(f1['peak_cuda_mb'], f2['peak_cuda_mb']),
           'free_gb_now': round(OS.free_gb(), 2), 'fits': 2, 'note': 'fit 1 is a real Source cell reused by the run; fit 2 re-trains a cached transfer Test cell'}
    res['status'] = 'PASS' if (rc == 0 and rc2 == 0 and bitwise and res['e_equal']) else 'FAIL'
    write(wd / 'wiring_result.json', res)
    print('WIRING', res['status'], json.dumps({k: res[k] for k in ('second_fit_in_same_process_bitwise_equal_transfer_model', 'e_equal', 'train_seconds', 'batch_wall_seconds',
