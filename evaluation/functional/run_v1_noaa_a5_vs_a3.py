...
判定（预注册）：
  ...
  ADHERENCE_GAP            : 检索到但行为不变（且 Memory 给出明确单向
                             可行动建议而 Agent 忽略）
  NO_INCREMENTAL_MEMORY_SIGNAL : 冲突/模糊 Memory（无明确单向指令）下
                             行为与 A3 相同——Memory 未产生增量信号
...
REPORT_REL = E2 / "w1_noaa_a5_vs_a3_report.json"
...
def _gain_series(sid, op, origin, roster_full, values, cfg, params=None):
    # 参数绑定修复（用户核查 2026-08-13）：必须用实际 winner params
    # （如 impute_linear strength=0.5）——_default_params 的 strength=1.0
    # 与 baseline 完整线性插值等价，会把真实效用测成 0
    compiled = v1.make_compiled(op, params or _default_params(op, 7))
    try:
    ...
    for sid in series_ids[:12]:
        ...
        for origin in SRC_ORIGINS:
            ...
            eps.append(build_episode(
                ...
                domain_namespace="nn5",  # Source 数据域修复（用户核查）
                context_summary={
                    "local_pattern": {
...
def main() -> int:
    ...
    root = PROJECT_ROOT
    # Target 装置：NOAA（hourly period 24——跨数据集 Target——feasibility
    # scan 发现 missingness Context + CLEAR_POSITIVE 方向）
    cfg = dict(v6.DATASET_CONFIGS["noaa"])
    ...
    roster_full = ([{"series_uid": s, "role": "train"}
                   ...
                   + [{"series_uid": s, "role": "eval"}
                      for s in series_ids[12:]])
    # Source：NN5 impute_fft 经验（跨数据集注入全部 24 条 prequential
    # 历史——origin<672；跨数据集无语义泄漏——NN5 Source 时间上全部
    # 先于 NOAA Target 发生）
    nn5_cfg = dict(v6.DATASET_CONFIGS["nn5"])
    nn5_roster, nn5_values = v6._fixed_roster(root, nn5_cfg)
    nn5_ids = [r["series_uid"] for r in nn5_roster]
    nn5_rf = ([{"series_uid": s, "role": "train"} for s in nn5_ids[:12]]
              + [{"series_uid": s, "role": "eval"} for s in nn5_ids[12:]])
    source_eps_all = _build_source_episodes(root, nn5_rf, nn5_values,
                                            nn5_cfg, nn5_ids)
    source_eps = [e for e in source_eps_all
                  if int((e.context_summary or {}).get("support_origin")
                         or 0) < 672]
    # Target series：公开 Context 冻结（feasibility 同判据——missing>0
    # + impute_fft 可执行——不读 gain）
    target_sid = None
    for sid in series_ids:
        arr = values[sid]
        if len(arr) < DELAYED_ORIGIN + 48:
            continue
        pub = extract_public_features(arr[:TARGET_ORIGIN],
                                      task_kind="forecast")
        if (pub.get("missing_fraction") or 0.0) > 0.0:
            target_sid = sid
            break
    if target_sid is None:
        report_tmp = {"verdict": "NO_TARGET_CONTEXT_PREVALENCE"}
        print(json.dumps(report_tmp, ensure_ascii=False))
        return 0
    ...
    def _run_arm(arm: str, memory_eps: tuple) -> dict[str, Any]:
        ...
        r = run_online_round(
            ...
            domain=f"nn5_p2w4_{arm}", period=24, fast_features=feats,
        ...
        # 用户核查修复（2026-08-13）：probe 保留 kind/passed——
        # verifier_rejected ≠ probe；只有 kind=probe + passed + gain 有限
        # 才算合法 Action
        probes = [(p["candidate_id"], p.get("gain"), p.get("kind"),
                   p.get("passed"))
                  for p in r.actual_probed_programs]
        legal = [p for p in probes if p[2] == "probe" and p[3]
                 and p[1] is not None]
        invalid = [p for p in probes if p[2] == "verifier_rejected"]
        gains = [p[1] for p in legal]
        return {"probes": probes,
                "memory_resolution_status": mem_status,
                "legal_actions": [p[0] for p in legal],
                "invalid_proposals": [p[0] for p in invalid],
                "first_pos_probes": next(
                    (i + 1 for i, p in enumerate(probes)
                     if p[2] == "probe" and p[1] is not None and p[1] >= M),
                    None),
                "neg_probes": sum(1 for p in probes
                                  if p[2] == "probe"
                                  and p[1] is not None and p[1] < -M),
                "harm": sum(1 for p in probes
                            if p[2] == "probe"
                            and p[1] is not None and p[1] < -M),
                "abstained": len(legal) == 0 and not invalid,
                "chosen_op": (legal[0][0] if legal else None),
    ...
    # delayed utility（用户核查修复：用实际 Program steps（winner_program
    # ——material 正 winner）评价——不从 candidate ID 字符串猜 Operator）
    for arm, res in (("A3", a3), ("A5", a5)):
        wp = res.get("winner_program")
        if wp:
            op = str(wp[0].get("op")) if isinstance(wp[0], dict) else None
            res["chosen_op_clean"] = op
            res["delayed_gain"] = _gain_series(
                target_sid, op, DELAYED_ORIGIN, roster_full, values, cfg,
                params=(wp[0].get("params")
                        if isinstance(wp[0], dict) else None))
        else:
    ...
    elif a5["invalid_proposals"]:
        # 用户核查：verifier 拒绝 = Memory 引导了非法提案
        verdict = "INVALID_MEMORY_GUIDED_PROPOSAL"
    elif a5["legal_actions"] and all(
            g is None for _c, g, _k, _p in a5["probes"]
            if _k == "probe" and _p):
        verdict = "EVALUATION_UNAVAILABLE"
    elif ambiguous_memory and not behavior_changed:
        # 用户裁决 2026-08-13：A5 收到的是冲突 Memory（正负同 workflow
        # 渲染为 AMBIGUOUS 块——无明确单向指令可遵循）→ 行为与 A3 相同
        # = Memory 未产生增量信号——不是 ADHERENCE_GAP（后者要求
        # Memory 给出明确单向可行动建议而 Agent 忽略）
        verdict = "NO_INCREMENTAL_MEMORY_SIGNAL"
