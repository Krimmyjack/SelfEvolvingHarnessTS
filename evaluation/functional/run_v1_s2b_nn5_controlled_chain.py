"""S2B_NN5_CONTROLLED_CHAIN（2026-08-13：S2 development follow-up——
仅改变信息边界——用户裁决方案 2：向 Slow Agent 开放 D_patch/Support
数值证据（两个候选及其 D_patch gain），绝不开放 D_heldin/D_delayed。
S2（SINGLE_SUPPORT_NO_MANIFEST）报告保留不覆盖。

信息边界修正（用户裁决）：
  - D_patch 的 Action–Response 数值 = Agent 应当读取的公开证据——
    分类与 Slow Edit 使用同一份公开 Capsule（避免前者看得见后者
    看不见）；
  - 最多开放两个候选及其 D_patch gain（@632）；
  - D_heldin（@728）/ D_delayed（@680）仍由 Runtime 独立决定是否
    激活 Skill——Agent 不可见；
  - 只允许重跑一次；失败即关闭该切片，不继续调 Prompt。

装置事实（同 S2——数据驱动修正）：NN5 长度 791 → held-in 776 超长改
728；repair_level_shift @728=+0.261 正向 → 组不成形 → 单条路径。

链（单条路径）：632 失败 → 五类选择题 + Slow Edit 同一公开 Capsule
（含两候选 D_patch gain）→ case-0004（S2 已写入——复用）→ Slow Agent
依据 D_patch 证据选择 Typed Edit → Support replay @632 门 → held-in
@728（Runtime 门）→ delayed @680（Runtime 门）→ approved → 采用双
口径 → removal 对照。

verdict（同 S2 预注册组合式 + SINGLE_SUPPORT_NO_MANIFEST 分支）：
  CONTROLLED_BATCH_EVOLUTION_CHAIN_PASS / LOCAL_SKILL_MECHANISM_PASS+
  AGENT_ADHERENCE_FAIL / RETRIEVAL_BINDING_EXECUTION_FAILURE /
  SINGLE_SUPPORT_NO_MANIFEST（再次弃权——关闭切片）/ PROTOCOL_FAILURE

用法：
  python evaluation/functional/run_v1_s2b_nn5_controlled_chain.py
"""
...
REPORT_REL = E2 / "w1_s2b_nn5_controlled_chain_report.json"
