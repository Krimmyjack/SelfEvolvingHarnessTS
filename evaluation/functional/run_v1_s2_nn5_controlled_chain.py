"""S2_NN5_CONTROLLED_CHAIN（2026-08-13：受限 Case 框架首次完整链验证——
NN5 missingness 受控正例——development positive control——零新 Claim。
预注册：docs/S2_NN5_CONTROLLED_CHAIN_PREREGISTRATION.md + 本文件装置修正）。

装置事实（运行前实测——数据驱动修正，全部在预注册 origin 表内）：
  - NN5 series 长度 = 791 → origin 776 + horizon 48 = 824 超长 →
    776 仪器失败（预注册 held-in 776 为装置错误，如实修正）。
  - repair_level_shift @728 = +0.261（正向）→ 组不成形（预注册已预见
    "如实记组不成形并走单条路径"分支）。
  - 修正：单条路径（632）+ held-in/采用 = 728（预注册表内 origin——
    唯一可评估后延迟窗）+ delayed = 680（已暴露核对）。

链（单条路径）：632 失败（已暴露核对 −0.0789）→ 五类选择题
（allowed=[WORKFLOW_DECISION_ERROR]）→ Case reconciliation
（NEW_CASE → Runtime 写 case-0004）→ Slow Agent 一个 Typed Edit
（白名单 [impute_ar, impute_ssm]——不硬编码）→ Support replay @632 门
→ held-in @728（≥ −M）→ delayed @680（已暴露核对）→ approved
（Skill 写入）→ 采用双口径（LLM 入口 + 确定性入口分开判定）@728 →
removal 对照。
...
verdict（预注册组合式）：
  ...
  GROUP_NOT_FORMED_SINGLE_PATH_* : 组不成形走单条路径后的门结果
  PROTOCOL_FAILURE

用法：
  python evaluation/functional/run_v1_s2_nn5_controlled_chain.py
"""
...
REPORT_REL = E2 / "w1_s2_nn5_controlled_chain_report.json"
...
def _card_for(group, capsule, case_summary, selectable, single=False):
    return {
        ...
        "instruction": (
            ...
            + ". The fault-selection step classified this as "
              "WORKFLOW_DECISION_ERROR (a positive replacement candidate "
              "was measured to exist, yet the initial entry did not "
              "propose it) and reconciled it to a Problem Case. Now "
              "choose exactly one typed patch from typed_patch_options "
              "whose frozen program is the best replacement, or declare "
              "no_proposal with reason_code insufficient_public_evidence "
              "if you cannot justify any. The deterministic Support "
              "replay gate verifies your patch — you do not approve your "
              "own edit. Do not invent patch_ids: use the whitelist.",
        ),
    }
...
def main() -> int:
    ...
    items = ([{"id": f"ep:{TS}", "desc": "repair_level_shift Support gain",
             ...
             + [{"id": f"alt:{op}@{TS}",
                ...
                for op, g in impute_gains.items()])
    counter_cls = smoke.CountingClient(
        openai.OpenAI(api_key=api_key, base_url=smoke.BASE_URL, timeout=120))
    prompt = (TAXONOMY_TEXT + "\n\nallowed_fault_types (Runtime, measured): "
              + json.dumps(selectable) + "\n\nGROUP EVIDENCE:\n"
              + "\n".join(f"- {it['id']}: {it['desc']} = {it['gain']}"
                          for it in items)
              + "\n\nEXISTING PROBLEM CASES:\n- case-0001 [WORKFLOW_SUPPLY_GAP]"
              " outlier/winsorize — no common positive headroom\n- case-0002"
              " [SCOPE_MEMORY_RISK_ERROR] T117 — hampel support+ delayed-\n"
              "- case-0003 [NO_ACTIONABLE_FAULT] T105 — action space empty"
              "\n\n" + OUTPUT_RULES)
    ans = _classification_call(api_key, counter_cls, prompt)
    ft = str(ans.get("fault_type") or "")
    act = str(ans.get("proposed_case_action") or "")
    refs = ans.get("evidence_refs") or []
    chosen_ft, err = (classify_group(evidence, ft) if ft
                      else ("INSUFFICIENT_EVIDENCE", "no answer parsed"))
    cls_checks = {
        "structure": bool(ft and isinstance(refs, list) and act in
                          CASE_ACTIONS),
        "no_masked_selection": bool(ft in selectable
                                    or (not selectable and ft in GUARDS)),
        "refs_resolvable": bool(refs and set(refs) <=
                                {it["id"] for it in items}),
    }
    report["classification"] = {"selectable": selectable, "answer": ans,
                                "normalized": chosen_ft,
                                "classify_error": err, "checks": cls_checks,
                                "llm_calls": counter_cls.calls}
    ...
    report["reconciliation"] = {"fault_type": chosen_ft, "matched":
                                ...
                                "action": action}
    new_id = "case-0004"
    if action == "NEW_CASE" and new_id not in {c["case_id"]
    ...
    if ev.get("stage") != "pending":
        report["verdict"] = ("SINGLE_SUPPORT_" + ev.get("stage",
                                                        "REJECTED").upper())
    ...
    if llm_chosen and runtime_adopted:
        verdict = "CONTROLLED_BATCH_EVOLUTION_CHAIN_PASS"
