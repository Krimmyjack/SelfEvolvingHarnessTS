...
class P6EditError(RuntimeError):
...
def apply_edit(state: P6HarnessState, edit: Any) -> P6HarnessState:
    ...
    if reason is not None:
        raise P6EditError(f"edit 被拒（{edit.to_dict().get('kind', '?')}）：{reason}")
...
def default_state(expected_total_K: int = H0_EXPECTED_TOTAL_K) -> P6HarnessState:
